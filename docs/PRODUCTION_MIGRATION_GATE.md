...
Expanding the entire spec to WebKit also exposed an offline-emulation limitation on both Windows and Linux. `context.setOffline(true)` rejects navigation with an internal WebKit error before the service worker can return its offline page. A minimal worker that always returns synthetic HTML reproduces that engine behavior. The service-worker regression therefore uses an isolated loopback reverse proxy for the actual application, then disconnects that origin at the network layer. It still executes the shipped worker and asserts old-cache cleanup, no onboarding caching, the real offline page, and rejection of private onboarding navigation despite a seeded stale cache. No response or worker is mocked, and no browser is skipped.
