...
| B Business policies | Structured draft/review/publish API/UI, immutable revisions and booking snapshots on public, checkout, confirmation, owner, customer, guest management and support surfaces. Real SQL verifies old evidence after publication plus paid/no-charge combined checkout. Policy enum values translate while exact original prose is preserved. | AUTOMATED ONLY |
...
Mandatory release-candidate CI includes source/core checks, all browser tests across three independent shards, clean database verification, security audits, TypeScript/lint/build and the separate off-state browser suite. All browser shards must pass. The original production migration dispatch guards remain intact; no workflow was manually dispatched.
