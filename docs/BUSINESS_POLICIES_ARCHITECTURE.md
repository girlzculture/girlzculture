...
The current published revision is rendered near the bottom of each public profile, headed with the business name. Refund & Service Satisfaction and Cancellation / Rescheduling / No-show have named sections. Platform legal/payment links remain separate. Historical revisions without the two additive refund fields remain immutable and valid; opening a new draft supplies defaults without rewriting old evidence.
...
Checkout now requires two separate unchecked acknowledgements: the exact business revision (when published) and the existing platform agreement. Opening policy details does not discard the booking form. The server rejects missing acknowledgements or a stale revision before contacting Stripe. Immutable evidence includes revision/version/snapshot, business acceptance timestamp, platform acceptance timestamp, booking identity, authenticated user ID or guest name/email, locale, channel and acknowledgement version. A business with no published policy has explicit no_published_policy evidence, not an invented agreement.

Business-added appointments store business_added_not_accepted with null acceptance timestamps/revision and an empty snapshot. They cannot claim a customer account, accepted platform/business policies, marketplace payment or customer conversation. The new forward migration is 20260914113932_p0_operational_calendar_and_policy_acceptance.sql.
