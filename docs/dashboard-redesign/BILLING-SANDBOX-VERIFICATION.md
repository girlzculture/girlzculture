...
Complete one real hosted Checkout flow using public Stripe test data, the actual app completion/readback path, and the correct existing customer/subscription. Verify the new masked/default method after return and reload, and leave that replacement saved for the intended subsequent subscription payments. Preserve the price, plan, schedule, customer defaults and every other commercial field. Verify no app-initiated invoice or payment; label any independent provider activity honestly. Test cancellation separately and require it to preserve the then-current default. Review event delivery against an isolated handler before completing setup or updating the subscription.

On a lost response, reconcile the original Session, SetupIntent, durable application attempt and authoritative provider state; do not blindly create another attempt or undo a successful replacement. Mandatory restoration of the former default and a separate private recovery store are not founder requirements. Those proposed prerequisites have been removed. Existing application attempt/stage/idempotency recovery remains required and unchanged. Only sanitized verdicts, counts and hashes may enter this public repository's workflow artifacts; never upload raw provider objects, private association records, Checkout URLs or credentials.

Those operations are intentionally unavailable in this verifier. Full billing provider acceptance remains incomplete. Scheduled-subscription application support has separate migration178/187 and local regression evidence; this sandbox harness does not yet exercise that provider path. The existing sandbox key can be reused for the later reviewed phase; a second credential request is unnecessary.

### Provider-free successful-save prerequisite

`scripts/billing-sandbox-success.mjs` is a disconnected helper for the existing verifier. It binds the reviewed SHA, sandbox association, durable attempt and app-created Session; requires a completed Session, the exact succeeded SetupIntent with `usage:off_session`, and its same-customer card. Before a write, it permits only the exact subscription-default field and app idempotency key, with fresh unchanged commercial, customer-default, invoice and PaymentIntent evidence. It then checks the actual completion result, fresh masked status, completed attempt and read-only terminal replay. The replacement remains saved. Changed or uncertain evidence fails; the helper does not retry or restore.

The focused tests invoke the real application begin/complete/status functions with simulated provider transport and RPC persistence. Thirteen new cases plus seven existing safety cases pass, including lost-response reconciliation without another write, null/non-card original defaults, mismatched setup usage, foreign associations, changed billing evidence and false success claims. Both existing CI safety command lists include the new test file; no mode, credential, permission or environment setting changed. Evidence: sibling `redesign-evidence/billing-success-before.log`, `billing-success-usage-before.log` and `billing-success-final.log`.

This helper has no network or hosted Checkout driver and returns `applicationReady:false`. Its caller-supplied authorization and read callbacks are contracts, not proof of protected workflow access or provider execution. Integration with the existing allowlisted transport, actual local database adapter and hosted Checkout/browser return remains outstanding, followed by protected real-provider acceptance. These simulated results do not establish real card entry, browser persistence, event isolation or no-charge provider behavior.
