...
WebKit service-worker requests were bypassing Playwright's explicit page-route fixtures and reaching the unrelated localhost fixture backend. P0 API-mocked suites now block service workers within those tests only; PWA/real-provider tests retain their behavior. No assertion, skip, retry or per-action timeout was used to conceal that failure. The expanded serial CI collection completed with 442 passed and five existing viewport skips, but exhausted its 60-minute step budget during completion. Normal CI now opts into two workers for that same complete collection through `PLAYWRIGHT_CI_WORKERS=2`; release-candidate CI retains three independent single-worker shards. The full-suite step remains in the existing `verify` job, preserving the production-migration evidence guard. Individual assertions/timeouts, retries=0 and the 60-minute step budget remain unchanged. Configuration/workflow changes also trigger release-candidate verification.
