...
The first release-candidate run also exposed a test setup race in WebKit under the development server. The description test changed server-rendered text before React hydrated it; the hydration diff showed the helper's `Traduit` text where the client expected the original description. This was separate from the reproduced post-hydration `removeChild` defect. Mutation tests now establish client readiness before replacing text, including a real expand/collapse cycle for the description. A delayed-JavaScript regression checks that translation waits for readiness. The original post-mutation interaction, content, hydration-error and runtime-error assertions remain in place; no error is suppressed and no retry or timeout is increased.
