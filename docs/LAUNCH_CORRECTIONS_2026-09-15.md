...
- **AUTOMATED ONLY — generated localization evidence:** The first rerun detected stale source-line references in the owner translation inventory. Regeneration changed only 15 line references after the assistant source comment; no text, exception or coverage requirement changed. The full source inventory and coverage checks pass for all 1,834 UI strings in all five languages.
- **PASS — preview exit interaction only:** In the deployed PR preview, the visible exit control is a POST form. Clicking it returns to onboarding, removes the demonstration banner, and keeps browsing closed after reload and a direct visit to `/salons`. This desktop Chrome check is not mobile/WebKit or authenticated AI acceptance, and the preview's exact deployed commit was not established. The repository's optional Supabase-backed preview smoke is intentionally disabled; its green workflow status is not evidence that preview data works.
