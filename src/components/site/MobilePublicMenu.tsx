...
const links = [
  ["Browse Styles", "/styles"], ["Find Salons", "/salons"], ["How It Works", "/how-it-works"],
  ["About Us", "/about"], ["Blog", "/blog"], ["Partner With Us", "/partner"], ["Social", "/social"],
] as const;
...
export default function MobilePublicMenu() {
  ...
  return <div ref={root} className="relative md:hidden"><button type="button" aria-label={open ? "Close navigation menu" : "Open navigation menu"} aria-expanded={open} onClick={() => setOpen((value) => !value)} className="inline-flex h-10 w-10 items-center justify-center rounded-xl text-ink">{open ? <X size={22}/> : <Menu size={22}/>}</button>{open ? <nav aria-label="Mobile navigation" className="absolute left-0 top-12 w-64 overflow-hidden rounded-[14px] border border-plum/10 bg-[#fffdfa] p-2 text-sm font-semibold text-ink shadow-[0_18px_42px_rgba(26,18,32,0.16)]">{links.map(([label,href]) => <Link key={href} href={href} onClick={() => setOpen(false)} className={`block rounded-[10px] px-4 py-3 hover:bg-blush/45 ${href === "/partner" ? "text-magenta" : ""}`}>{label}</Link>)}<div className="my-1 border-t border-plum/10"/><Link href="/login" onClick={() => setOpen(false)} className="block rounded-[10px] px-4 py-3 hover:bg-blush/45">Log in</Link><Link href="/login" onClick={() => setOpen(false)} className="block rounded-[10px] bg-magenta px-4 py-3 text-center text-white">Sign up</Link></nav> : null}</div>;
}
