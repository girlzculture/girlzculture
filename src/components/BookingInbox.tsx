...
export default function BookingInbox({ scope }: { scope: AuthScope }) {
  ...
  async function send(event: FormEvent) {
    ...
    try {
      const headers = await authHeaders();
      const response = await fetch("/api/messages", { method: "POST", headers: { ...headers, "Content-Type": "application/json" }, body: JSON.stringify({ booking_id: selectedId, body: draft }) });
      const body = await response.json();
  ...
  return <section className="grid min-h-[610px] overflow-hidden rounded-[18px] border border-plum/10 bg-white xl:grid-cols-[330px_1fr]">
    ...
    <div className="flex min-h-[520px] flex-col"><header className="border-b border-plum/10 p-5"><h3 className="font-serif text-xl text-plum">{bookingLabel(selected.booking)}</h3><p className="mt-1 text-[11px] text-ink/55">{role === "customer" ? selected.booking.salon?.name : selected.booking.guest_name} · {new Date(selected.booking.appointment_datetime).toLocaleString()}</p></header><div className="flex-1 space-y-3 overflow-y-auto bg-cream/35 p-5">{messages.map((message) => {
      ...
      return <article key={message.id} className={`max-w-[82%] rounded-[14px] px-4 py-3 text-sm ${mine ? "ml-auto bg-plum text-white" : "bg-white text-ink shadow-sm"}`}><p className="whitespace-pre-wrap break-words">{message.body}</p><small className={`mt-2 block text-[9px] ${mine ? "text-white/60" : "text-ink/40"}`}>{message.sender_role === "customer" ? "Customer" : message.sender_role === "salon" ? "Salon" : "Girlz Culture Support"} · {new Date(message.created_at).toLocaleString()}</small></article>;
    })}{!messages.length ? <p className="py-20 text-center text-sm text-ink/45">No messages yet. Ask a question about this appointment.</p> : null}</div>{scope !== "admin" ? <form onSubmit={send} className="border-t border-plum/10 p-4"><label className="sr-only" htmlFor="booking-message">Message</label><div className="flex gap-2"><textarea id="booking-message" value={draft} onChange={(event) => setDraft(event.target.value.slice(0, 2000))} rows={2} placeholder="Type a private booking message…" className="min-w-0 flex-1 resize-none rounded-[10px] border border-plum/15 p-3 text-sm outline-none focus:border-magenta" /><button disabled={sending || !draft.trim()} className="grid w-14 place-items-center rounded-[10px] bg-magenta text-white disabled:opacity-50" aria-label="Send message"><Send size={19} /></button></div></form> : <p className="border-t border-plum/10 p-4 text-center text-xs text-ink/50">Admin read-only view for support and safety.</p>}{notice ? <p className="border-t border-red-200 bg-red-50 p-3 text-xs text-red-700">{notice}</p> : null}</div>
  </section>;
}
