    ...
    <div className={`grid min-w-0 overflow-hidden rounded-2xl border border-plum/10 bg-white ${focused ? "" : "lg:grid-cols-[minmax(240px,30%)_minmax(0,1fr)]"}`}>
      ...
      {selected && <div className={`min-w-0 flex-col ${!mobileConversation ? "hidden lg:flex" : "flex"}`}>
        ...
        {scope!=="admin" && windowState.open && <form onSubmit={send} className="space-y-3 border-t p-4">
          ...
          <label htmlFor="booking-message" className="sr-only">{t("Message")}</label><div className="flex gap-2"><textarea id="booking-message" value={draft} disabled={conversationLoading} onChange={event => editDraft(event.target.value.slice(0,2000))} rows={3} placeholder={t("Type a private booking message…")} className="min-w-0 flex-1 resize-y rounded-xl border p-3 text-sm"/><button disabled={sending||conversationLoading||!draft.trim()} className="grid min-h-11 w-12 shrink-0 place-items-center rounded-xl bg-magenta text-white disabled:opacity-50" aria-label={t(translationPreview ? "Send original and previewed translation" : "Send message")}><Send size={19}/></button></div>
          <details><summary className="cursor-pointer text-sm text-magenta">{t("Preview translation")}</summary><div className="mt-2 flex flex-wrap items-center gap-2"><select value={targetLocale} onChange={event => {previewGeneration.current++;setTargetLocale(event.target.value);setTranslationPreview(null);}} aria-label={t("Translation language")} className="min-h-11 rounded-lg border bg-white px-2 text-sm">{["en","fr","es","zh-CN"].map(code=><option key={code} value={code} data-no-translate>{LOCALE_NAMES[code]}</option>)}</select><button type="button" disabled={sending||conversationLoading||!draft.trim()} onClick={() => void previewTranslation()} className="inline-flex min-h-11 items-center gap-2 rounded-lg border px-3 text-sm disabled:opacity-50"><Languages size={16}/>{t("Preview translation")}</button><p className="text-xs gc-text-secondary">{t("The original is always preserved. Nothing translated is sent until you preview and press Send.")}</p></div></details>
