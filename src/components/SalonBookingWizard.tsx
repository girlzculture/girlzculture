...
function StylePanel(props: Row) {
  ...
  return <div>
    ...
    <div className="mt-4 rounded-lg bg-blush/40 p-3">{props.promotionDiscount ? <><p className="flex justify-between text-[10px] text-ink/50"><span>Original price</span><span className="line-through">{money(props.subtotal)}</span></p><p className="mt-1 flex justify-between text-[10px] text-magenta"><span>{props.salonPromotion?.public_headline || props.salonPromotion?.title || "Offer"}</span><b>-{money(props.promotionDiscount)}</b></p></> : null}<p className="mt-1 flex justify-between"><b className="text-xs">Estimated Total</b><b className="text-plum">{money(props.total)}</b></p></div>
  </div>;
}
