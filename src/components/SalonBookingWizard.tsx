...
import { CalendarDays, Check, Clock3, LockKeyhole, ShieldCheck, Star } from "lucide-react";
...
const fallback = ["/images/braids-box.jpg", "/images/braids-cornrows.jpg", "/images/braids-knotless.jpg"];
...
function StylistPanel({ stylists, value, setValue }: { stylists: Row[]; value: string; setValue: (value: string) => void }) { const rows = [{ id: "any", name: "Any available stylist", bio: "We’ll assign an available professional without double-booking." }, ...stylists]; return <div className="space-y-2">{rows.map((stylist, index) => <button key={stylist.id} onClick={() => setValue(stylist.id)} className={`flex w-full items-center gap-3 rounded-[9px] border p-2 text-left ${value === stylist.id ? "border-magenta bg-blush/30" : ""}`}><SafeImage src={stylist.avatar_url} fallbackSrc={fallback[index % 3]} alt={stylist.name} className="h-11 w-11 rounded-lg object-cover" /><span className="min-w-0 flex-1"><b className="block text-xs">{stylist.name}</b><small className="text-[9px] text-ink/55">{stylist.bio || stylist.specialties?.join(" · ")}</small></span><input type="radio" checked={value === stylist.id} readOnly /></button>)}</div>; }
