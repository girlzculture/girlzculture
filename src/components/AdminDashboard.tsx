...
import ReferralCampaigns, { ReferralCampaignAccessDenied, ReferralCampaignEntry } from "@/components/admin/ReferralCampaigns";
...
const ADMIN_PLAN_FILTERS = [
  ...PLAN_ORDER.map((plan) => [plan, plan] as const),
  ["Basic", "Basic (legacy)"] as const,
];
...
export default function AdminDashboard({ section, recordId, returnTo, acceptanceData, children }: { section: AdminSection; preview?: boolean; recordId?: string; returnTo?: string; acceptanceData?: AdminAcceptanceData; children?: React.ReactNode }) {
  ...
  return <AdminShell section={section} access={access} inboxCounts={inboxCounts} acceptance={acceptance}>{acceptance ? null : <RoleSessionBoundary scope="admin" />}
    ...
    <div onClickCapture={rememberAdminListScroll}>
      {children || <AdminSectionView section={section} isSuperAdmin={access === null} recordId={recordId} returnTo={returnTo} data={data} overviewMetrics={overviewMetrics} selected={selected} setSelected={setSelected} decide={decide} update={update} onCreated={load} onTicketRead={(mode) => setInboxCounts((counts) => ({ ...counts, [mode]: Math.max(0, counts[mode] - 1) }))} />}
    </div>
  </AdminShell>;
}
...
function AdminSectionView({ section, isSuperAdmin, recordId, returnTo, data, overviewMetrics, selected, setSelected, decide, update, onCreated, onTicketRead }: { section: AdminSection; isSuperAdmin: boolean; recordId?: string; returnTo?: string; data: DataState; overviewMetrics: OverviewMetrics | null; selected: Row | null; setSelected: (row: Row) => void; decide: (id: string, decision: "approve" | "reject" | "activate") => void; update: (table: string, id: string, changes: Row) => Promise<void>; onCreated: () => Promise<void>; onTicketRead: (mode: "support" | "complaints") => void }) {
  ...
  if (section === "settings" && recordId === "referrals") return isSuperAdmin ? <ReferralCampaigns/> : <ReferralCampaignAccessDenied/>;
  ...
  const props = { ...safeData, isSuperAdmin, selected, setSelected, decide, update, onCreated };
  switch (section) {
  ...
  return <div className="grid min-w-0 gap-5 xl:grid-cols-[1.2fr_.8fr]">
    <section className="min-w-0 rounded-[14px] border border-plum/10 bg-white p-4">
      ...
      <DataTable headers={["Business Name", "Owner / Contact", "Submitted", "Status", "Review"]}>{rows.map((application: Row) => <tr key={application.id} onClick={() => p.setSelected(application)} className="border-b border-plum/10 hover:bg-blush/20">
        ...
        <Td><div className="flex flex-col items-start gap-2"><Link href={`/admin/submissions/${application.id}`} onClick={(event) => event.stopPropagation()} className="font-bold text-magenta">Full details</Link>{application.status === "Offboarded" ? <Link href="/admin/engine?category=data_management" onClick={(event) => event.stopPropagation()} className="text-[10px] font-bold gc-text-danger">Archive / delete test record</Link> : null}</div></Td>
      </tr>)}</DataTable>
    </section>
          ...
          return (
            <article key={review.id} className={`rounded-xl border p-4 ${disputed ? "border-red-200 bg-red-50/60" : "border-plum/10 bg-white"}`}>
              ...
              <div className="mt-4 grid gap-3 rounded-lg bg-cream p-3 text-xs sm:grid-cols-2 lg:grid-cols-4">
                ...
                <div><b>Deposit</b><p className="mt-1 gc-text-primary">{booking ? `${money(Number(booking.deposit_amount || 0))} · ${booking.deposit_status || "Unknown"}` : "Unavailable"}</p></div>
              </div>
              ...
              <div className="mt-4">
                {activeReview === review.id ? (
                  <form onSubmit={(event) => { event.preventDefault(); void moderate(String(review.id)); }} className="rounded-lg border border-magenta/20 bg-blush/30 p-3">
                    ...
                    <div className="mt-3 flex gap-2"><button disabled={saving} className="min-h-10 rounded-lg bg-magenta px-4 text-xs font-bold text-white gc-disabled-control">{saving ? "Saving…" : "Save audited decision"}</button><button type="button" onClick={() => { setActiveReview(null); setReason(""); }} className="min-h-10 rounded-lg border border-plum/15 px-4 text-xs">Cancel</button></div>
                  </form>
...
function Badge({ value }: { value?: string }) { const label = value || "Pending"; const good = /active|verified|published|confirmed|approved/i.test(label); const bad = /reject|suspend|flag|remove/i.test(label); return <span className={`whitespace-nowrap rounded-full px-2 py-1 text-[9px] ${good ? "bg-emerald-50 gc-text-success" : bad ? "bg-red-50 gc-text-danger" : "bg-amber/15 gc-text-warning"}`}>{label}</span>; }
