...
export default function WorkspaceCalendar({ events, timeZone = "UTC", initialView = "week", initialDate, title = "Appointment calendar", compactHeader = false, eventTitleFirst = false }: { events: WorkspaceEvent[]; timeZone?: string; initialView?: View; initialDate?: string; title?: string; compactHeader?: boolean; eventTitleFirst?: boolean }) {
        ...
        {days.map(day => {
          ...
          return <section key={key} aria-label={formatDate(day, { dateStyle: "full", timeZone: "UTC" })} className={`min-w-0 border-b border-r border-border p-2 ${view === "month" ? "min-h-28" : "min-h-20 sm:min-h-64"} ${today ? "bg-subtle" : "bg-white"}`}>
            ...
            <div className="space-y-2">{appointments.map(event => {
              const eventTitle = <b data-no-translate className={`${eventTitleFirst ? "mb-1" : "mt-1"} block break-words text-sm`}>{event.title}</b>;
              const content = <>{eventTitleFirst ? eventTitle : null}<time className="block text-xs font-bold">{calendarDayKey(new Date(event.start), timeZone) < key ? t("Continues") : formatDate(event.start, { timeZone, hour: "numeric", minute: "2-digit" })}</time>{event.end ? <span className="block text-xs">{t("Ends {value0}", { value0:formatDate(event.end, { timeZone, month:"short", day:"numeric", hour:"numeric", minute:"2-digit" }) })}</span> : null}{eventTitleFirst ? null : eventTitle}{event.subtitle ? <span data-no-translate className="mt-1 block break-words text-xs">{event.subtitle}</span> : null}{event.status ? <span className="mt-2 block text-xs font-semibold">{t(event.status)}</span> : null}</>;
