...
export default function ContactSupportForm({categories}:{categories:string[]}) {
  ...
  return <form onSubmit={submit} className="rounded-3xl border border-plum/10 bg-white p-5 shadow-[0_18px_55px_rgba(13,17,20,.08)] sm:p-8">
    <div className="grid gap-5 sm:grid-cols-2"><Field label="Name"><input required value={form.name} onChange={(event) => update("name", event.target.value)} /></Field><Field label="Email"><input required type="email" pattern={EMAIL_PATTERN} title="Enter a valid email address such as name@example.com" placeholder="name@example.com" value={form.email} onChange={(event) => update("email", event.target.value)} /></Field><Field label="Subject"><input required value={form.subject} onChange={(event) => update("subject", event.target.value)} /></Field><Field label="Category"><select required value={form.category} onChange={(event) => update("category", event.target.value)}><option value="">Choose a category</option>{categories.map(category=><option key={category}>{category}</option>)}</select></Field></div>
    ...
    {notice ? <p aria-live="polite" className={`mt-4 rounded-xl p-4 text-sm ${sent ? "bg-green-50 gc-text-success" : "bg-red-50 gc-text-danger"}`}>{sent ? <CheckCircle2 className="mr-2 inline" size={17}/> : null}{notice}</p> : null}
    <button disabled={saving} className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-magenta px-7 py-4 font-bold text-white gc-disabled-control"><Send size={18}/>{saving ? "Sending…" : "Send support request"}</button>
  </form>;
}
