...
export default function ContactSupportForm() {
  ...
  return <form onSubmit={submit} className="rounded-3xl border border-plum/10 bg-white p-5 shadow-[0_18px_55px_rgba(26,18,32,.08)] sm:p-8">
    <div className="grid gap-5 sm:grid-cols-2"><Field label="Name"><input required value={form.name} onChange={(event) => update("name", event.target.value)} /></Field><Field label="Email"><input required type="email" value={form.email} onChange={(event) => update("email", event.target.value)} /></Field><Field label="Subject"><input required value={form.subject} onChange={(event) => update("subject", event.target.value)} /></Field><Field label="Category"><select value={form.category} onChange={(event) => update("category", event.target.value)}><option>Booking question</option><option>Payment or deposit</option><option>Salon experience</option><option>Account help</option><option>Safety concern</option><option>Partnership question</option><option>Other</option></select></Field></div>
