...
export default function SalonDiscovery({ initialSalons, initialTotal, initialStyle = "", initialLocation = "", initialOrigin = null }: Props) {
  ...
  return <>
    {!origin?<LocationPrompt value={displayedLocation} onChange={(value)=>{setEditingLocation(true);setLocationText(value);setManualLocation(null);}} onResolved={resolveLocation} onUseDevice={requestDeviceLocation} permissionError={customerLocation.permissionError}/>:<>
      <div className="relative z-[70] mt-3 max-w-[340px]"><StyleAutocomplete value={style} onChange={setStyle} onLocation={resolveLocation} placeholder="Filter nearby salons by service" className="rounded-[9px] border border-plum/15 bg-white px-3"/></div>
...
function LocationPrompt({value,onChange,onResolved,onUseDevice,permissionError}:{value:string;onChange:(value:string)=>void;onResolved:(value:CustomerLocation|null)=>void;onUseDevice:()=>Promise<void>;permissionError:string}) { return <section className="mt-5 rounded-[18px] border border-plum/10 bg-blush/70 p-5 text-center sm:p-7"><MapPin className="mx-auto text-magenta" size={28}/><h2 className="mt-3 font-serif text-2xl text-plum">Choose a location to see nearby salons</h2><p className="mx-auto mt-2 max-w-xl text-sm leading-6 text-ink/70">Type a city, neighborhood or ZIP, or share your device location. Location permission is never requested automatically.</p><div className="relative z-[70] mx-auto mt-4 max-w-xl rounded-[10px] bg-white text-left"><LocationAutocomplete name="discovery_location_query" value={value} onChange={onChange} onResolved={onResolved} placeholder="City, neighborhood, or ZIP" className="rounded-[10px] border border-plum/15 px-3"/></div>{permissionError?<p role="alert" className="mx-auto mt-2 max-w-xl text-left text-[11px] text-red-700">{permissionError}</p>:null}<div className="mt-3 flex flex-wrap justify-center gap-3"><button type="button" onClick={()=>void onUseDevice()} className="inline-flex min-h-11 items-center gap-2 rounded-[9px] bg-magenta px-5 text-sm font-bold text-white"><LocateFixed size={16}/>Use my location</button><Link href="/styles" className="inline-flex min-h-11 items-center rounded-[9px] border border-magenta px-5 text-sm font-bold text-magenta">Browse styles</Link></div></section> }
