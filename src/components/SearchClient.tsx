          ...
          : salons.map((s) => (
              <div key={s.id} className="rounded-lg border border-plum/10 bg-white p-4 shadow-sm">
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <div className="flex flex-wrap items-center gap-2"><span className="font-semibold text-plum">{s.name}</span><span className={`rounded-full px-2 py-1 text-xs font-bold ${isSalonClosedToday(s)?"bg-red-100 gc-text-danger":"bg-blush/55 text-plum"}`}>{getSalonStatusLabel(s)}</span></div>
