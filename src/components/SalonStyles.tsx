      ...
      {styleCards.map((card) => {
        ...
        return (
          <div key={card.id} className="border-b border-plum/10 last:border-b-0">
            ...
            >
              <span data-no-translate="true" className="min-w-0 text-[12px] font-semibold sm:text-[13px]"><span className="block truncate">{card.style.name || "Style"}</span>{card.offer ? <span className="mt-1 inline-flex rounded-full bg-amber/20 px-2 py-0.5 text-[8px] font-bold gc-text-warning">{promotionLabel(card.offer.promotion, card.offer.price.discount)}</span> : null}</span>
