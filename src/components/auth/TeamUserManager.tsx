...
type TeamUser = { id: string; name?: string; email: string; role?: string; status?: string; permissions?: Record<string, boolean>; is_super_admin?: boolean; stylist_id?: string | null };
...
export default function TeamUserManager({ scope }: { scope: TeamScope }) {
  ...
  const load = useCallback(async () => {
    try { const response = await fetch(endpoint, { headers: await auth(), cache: "no-store" }); const body = await response.json(); if (!response.ok) throw new Error(body.error); setUsers(body.users || []); setStylists(body.stylists || []); }
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const payload = { id: selected?.id, name: form.get("name"), email: form.get("email"), role: form.get("role"), stylist_id: form.get("stylist_id"), status: form.get("status"), permissions };
