...
const salonPermissions = [["overview","Overview"],["my_page","My Page"],["photos","Photos"],["styles","Services & Pricing"],["stylists","Team"],["products","Products"],["availability","Availability & Calendar"],["bookings","Bookings"],["reviews","Reviews"],["earnings","All business finances"],["earnings_own","Own professional earnings only"],["finance_log","Record sales and payments"],["finance_manage","Manage finance records"],["client_history","Client visit history"],["client_formulas","Client formulas"],["client_notes","Client preferences and notes"],["client_cautions","Client cautions"],["client_photos","Private client photos"],["client_spend","Client spend history"],["client_edit","Edit permitted client fields"],["promotions","Promotions"],["settings","Settings & Team"]] as const;
...
export default function TeamUserManager({ scope, initialUserId, showBackLink = true }: { scope: TeamScope; initialUserId?: string; showBackLink?: boolean }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    setMessage("");
    setSubmitting(true);
    // Capture the form before awaiting; React clears currentTarget after the async boundary.
  ...
  const editor = <form key={selected?.id || "new"} onSubmit={submit} className="rounded-[14px] border border-plum/10 bg-white p-5">
      ...
      <div className="mt-5 grid gap-4 sm:grid-cols-2">
        ...
        <Field name="phone" label="US mobile phone" type="tel" inputMode="tel" pattern={US_PHONE_PATTERN} title="Enter a valid US mobile number" defaultValue={selected?.phone}/>
        <label className="text-sm font-semibold">Role<select name="role" defaultValue={selected?.role || (scope === "admin" ? "Admin" : "Staff")} className="mt-2 w-full rounded-lg border border-plum/15 bg-white p-3 font-normal">{roleOptions.map((role) => <option key={role}>{role}</option>)}</select></label>
        <label className="text-sm font-semibold">Status<select name="status" defaultValue={["Inactive", "Suspended", "Revoked"].includes(selected?.status || "") ? "Inactive" : "Active"} className="mt-2 w-full rounded-lg border border-plum/15 bg-white p-3 font-normal"><option>Active</option><option>Inactive</option></select></label>
        {scope === "salon" ? <label className="text-sm font-semibold">Linked professional profile<select name="stylist_id" defaultValue={selected?.stylist_id || ""} className="mt-2 w-full rounded-lg border border-plum/15 bg-white p-3 font-normal"><option value="">Not a professional login</option>{stylists.map((stylist) => <option value={stylist.id} key={stylist.id}>{stylist.name}</option>)}</select></label> : null}
      </div>
      ...
      <button disabled={submitting} className="mt-5 rounded-lg bg-magenta px-7 py-3 font-bold text-white gc-disabled-control">{submitting ? "Saving…" : selected ? "Update User" : "Add User"}</button>
      {selected && !selected.is_super_admin ? <div className="mt-4 flex flex-wrap gap-2 border-t border-plum/10 pt-4">{scope === "admin" && selected.status === "Invited" ? <Action icon={MailPlus} label="Resend invitation" onClick={() => void adminAction(selected, "resend")}/> : null}{scope === "admin" && selected.status === "Active" ? <Action icon={ShieldOff} label="Suspend" onClick={() => void adminAction(selected, "suspend")}/> : null}{scope === "admin" && ["Inactive", "Suspended", "Revoked"].includes(selected.status || "") ? <Action icon={RefreshCw} label="Reactivate" onClick={() => void adminAction(selected, "reactivate")}/> : null}{scope === "admin" && selected.status !== "Revoked" ? <button type="button" onClick={() => void adminAction(selected, "revoke")} className="rounded-lg border border-red-300 px-3 py-2 text-xs font-bold gc-text-danger">Revoke</button> : null}<button type="button" onClick={() => void remove(selected)} className="flex items-center gap-1 rounded-lg border border-red-300 px-3 py-2 text-xs font-bold gc-text-danger"><Trash2 size={15}/>Remove access</button></div> : null}
    </form>;
...
function Status({ user }: { user: TeamUser }) { return <span className="rounded-full bg-blush px-3 py-1 text-xs font-semibold text-plum">{user.is_super_admin ? "Super Admin" : user.status || "Invited"}</span>; }
function Action({ icon: Icon, label, onClick }: { icon: typeof MailPlus; label: string; onClick: () => void }) { return <button type="button" onClick={onClick} className="flex items-center gap-1 rounded-lg border border-plum/15 px-3 py-2 text-xs font-bold text-plum"><Icon size={14}/>{label}</button>; }
function Field({ name, label, type = "text", defaultValue, readOnly = false, inputMode, pattern, title }: { name: string; label: string; type?: string; defaultValue?: string; readOnly?: boolean; inputMode?: "tel"; pattern?: string; title?: string }) { return <label className="text-sm font-semibold">{label}<input name={name} type={type} required defaultValue={defaultValue || ""} readOnly={readOnly} inputMode={inputMode} pattern={pattern} title={title} className="mt-2 w-full rounded-lg border border-plum/15 bg-white p-3 font-normal read-only:bg-cream/60"/></label>; }
