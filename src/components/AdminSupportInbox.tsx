...
type Ticket = {
  ...
  content_moderation_status?: string | null; content_moderation_reason?: string | null;
};
...
export default function AdminSupportInbox({
  ...
  return <div className="grid gap-5 xl:grid-cols-[390px_1fr]">
    <section className="rounded-[14px] border border-plum/10 bg-white p-4">
      ...
      <div className="mt-4 space-y-2">{visible.length ? visible.map((ticket) => <button key={ticket.id || ticket.subject || "ticket"} onClick={() => void openTicket(ticket)} className={`relative w-full rounded-xl border p-4 text-left ${selectedId === ticket.id ? "border-magenta bg-blush/30" : "border-plum/10"}`}>{!ticket.admin_read_at ? <span aria-label="Unread" className="absolute right-3 top-3 h-2.5 w-2.5 rounded-full bg-magenta" /> : null}<div className="flex items-center justify-between gap-2 pr-5"><b className="line-clamp-1 text-sm text-plum">{ticket.subject}</b><span className="rounded-full bg-blush px-2 py-1 text-[9px] font-bold text-magenta">{ticket.status}</span></div><p className="mt-1 text-xs text-ink/55">{ticket.requester_name} · {ticket.category}</p>{ticket.category === "Complaint" ? <span className={`mt-2 inline-flex rounded-full px-2 py-1 text-[9px] font-bold ${ticket.booking_verified ? "bg-emerald-100 text-emerald-800" : "bg-amber/15 text-amber-900"}`}>{ticket.booking_verified ? "Booking verified · counts in quality" : "Unverified · human review only"}</span> : null}{ticket.content_moderation_status === "Flagged" ? <span className="ml-2 mt-2 inline-flex rounded-full bg-red-100 px-2 py-1 text-[9px] font-bold text-red-800">Content review required</span> : null}<p className="mt-2 line-clamp-2 text-xs leading-5 text-ink/70">{ticket.message}</p></button>) : <p className="rounded-xl bg-blush/20 p-6 text-center text-sm text-ink/55">No {emptyLabel}s in this view.</p>}</div>
    </section>
    <section className="rounded-[14px] border border-plum/10 bg-white p-5">{selected ? <>
      ...
      {selected.content_moderation_status === "Flagged" ? <div role="alert" className="mt-4 rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-800"><b>Content review required</b><p className="mt-1">The safety filter flagged this report for human review{selected.content_moderation_reason ? ` (${selected.content_moderation_reason})` : ""}. The report was preserved so evidence is not lost.</p></div> : null}
