  ...
  return <section data-admin-record-landing className="rounded-[14px] border border-plum/10 bg-white p-4 sm:p-5">
    ...
    <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">{visible.length ? visible.map((ticket) => {
      ...
      return <Link key={ticket.id || ticket.subject || "ticket"} href={`/admin/${mode}/${ticket.id}?return=${encodeURIComponent(returnPath)}`} className="relative rounded-xl border border-plum/10 p-4 text-left transition hover:border-magenta hover:bg-blush/20">
        {!ticket.admin_read_at ? <span aria-label="Unread" className="absolute right-3 top-3 h-2.5 w-2.5 rounded-full bg-magenta"/> : null}
        ...
        {ticket.category === "Complaint" ? <span className={`mt-2 inline-flex rounded-full px-2 py-1 text-[9px] font-bold ${ticket.booking_verified ? "bg-emerald-100 text-emerald-800" : "bg-amber/15 text-amber-900"}`}>{ticket.booking_verified ? "Booking verified" : "Verification required"}</span> : null}
        {ticket.content_moderation_status === "Flagged" ? <span className="ml-2 mt-2 inline-flex rounded-full bg-red-100 px-2 py-1 text-[9px] font-bold text-red-800">Content review required</span> : null}
