...
const PLAN_FILTERS = [...PLAN_ORDER, "Basic (legacy)"] as const;
...
export default function AdminSubscriptionsDashboard({ salons, subscriptions, billingEvents, changeRequests }: {
  ...
  const matrix = useMemo(() => {
    ...
    filteredSubscriptions.forEach((subscription) => {
      ...
      const plan = normalizedPlan(subscription.tier || salon?.subscription_tier);
      const entry = groups.get(state) || { Starter: 0, Growth: 0, Premium: 0, "Basic (legacy)": 0, Total: 0 };
  ...
  return <div className="space-y-5">
    <section className="rounded-[14px] border border-plum/10 bg-white p-4">
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
        ...
        <label className="text-xs font-bold">Plan<select value={planFilter} onChange={(event) => setPlanFilter(event.target.value)} className="mt-1 min-h-10 w-full rounded-lg border border-plum/15 bg-white px-3"><option value="all">All plans</option>{PLAN_FILTERS.map((plan) => <option key={plan}>{plan}</option>)}</select></label>
    ...
    <section className="overflow-hidden rounded-[14px] border border-plum/10 bg-white">
      ...
      <div className="divide-y divide-plum/10 md:hidden">{matrix.length ? matrix.map(([state, values]) => <article key={state} className="p-4"><div className="flex items-center justify-between gap-3"><h3 className="font-serif text-lg text-plum">{state}</h3><b className="text-sm text-magenta">{values.Total} total</b></div><dl className="mt-3 grid grid-cols-2 gap-2 text-center text-xs">{PLAN_FILTERS.map((plan) => <div key={plan} className="rounded-lg bg-cream p-2"><dt className="text-[9px] font-bold uppercase tracking-wide text-ink/50">{plan}</dt><dd className="mt-1 font-serif text-lg text-plum">{values[plan]}</dd></div>)}</dl></article>) : <p className="p-8 text-center text-sm text-ink/50">No subscription records match these filters.</p>}</div>
      <div className="hidden overflow-x-auto md:block"><table className="w-full min-w-[680px] text-left text-xs"><thead className="bg-blush/25"><tr>{["State", ...PLAN_FILTERS, "Total"].map((header) => <th key={header} className="px-4 py-3">{header}</th>)}</tr></thead><tbody>{matrix.length ? matrix.map(([state, values]) => <tr key={state} className="border-t border-plum/10"><td className="px-4 py-3 font-bold">{state}</td>{PLAN_FILTERS.map((plan) => <td key={plan} className="px-4 py-3">{values[plan]}</td>)}<td className="px-4 py-3 font-bold">{values.Total}</td></tr>) : <tr><td colSpan={6} className="px-4 py-10 text-center text-ink/50">No subscription records match these filters.</td></tr>}</tbody></table></div>
    </section>
