  ...
  return <section className="rounded-[14px] border border-plum/10 bg-white/75 p-5 shadow-[0_8px_26px_rgba(26,18,32,.03)]">
    ...
    {!config ? <div className="mt-5 flex items-center gap-2 text-sm text-ink/60"><RefreshCw className="animate-spin" size={16}/>Loading lifecycle rules…</div> : <>
      <div className="mt-5 grid gap-2 sm:grid-cols-2 xl:grid-cols-3">
        ...
        <label className="rounded-xl border border-plum/10 bg-cream/50 p-3 text-xs font-semibold">Required gallery photos<input type="number" min="0" max="20" step="1" value={Number(config.required.gallery_photos || 0)} onChange={(event) => setConfig({ ...config, required: { ...config.required, gallery_photos: Number(event.target.value) } })} className="mt-2 min-h-10 w-full rounded-lg border border-plum/15 bg-white px-3"/></label>
      </div>
      <div className="mt-4 grid gap-3 sm:grid-cols-3">
        ...
        <label className="text-xs font-bold">Grace period days<input type="number" min="0" max="90" step="1" value={config.grace_period_days} onChange={(event) => setConfig({ ...config, grace_period_days: Number(event.target.value) })} className="mt-1 min-h-11 w-full rounded-lg border border-plum/15 bg-white px-3"/></label>
      </div>
