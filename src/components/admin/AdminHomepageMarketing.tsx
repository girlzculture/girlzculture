...
import { getVideoDuration, MAX_TRENDING_VIDEO_BYTES, optimizeTrendingVideo } from "@/lib/videoUploadClient";
...
export default function AdminHomepageMarketing({ salons }: { salons: Row[] }) {
  ...
  async function saveSection(section: Row) {
    ...
    if (!Number.isInteger(sortOrder) || sortOrder < 1 || sortOrder > 20) {
      setNotice("Enter a row order from 1 to 20.");
      return;
    }
    setBusy(true); setNotice("");
    try {
      const response = await fetch("/api/admin/marketing", { method: "POST", headers: { ...(await headers()), "Content-Type": "application/json" }, body: JSON.stringify({ kind: "section", ...section, sort_order: sortOrder }) });
      const body = await response.json();
  ...
  async function upload(event: FormEvent) {
    ...
    try {
      ...
      const initialDuration = await getVideoDuration(file);
      ...
      const processed = optimized.file;
      if (processed.size > MAX_TRENDING_VIDEO_BYTES) throw new Error("The optimized video is still over 25 MB. Export a shorter or lower-resolution clip.");
      const finalDuration = optimized.duration;
