...
export default function ErrorMonitoringManager() {
  ...
  async function load() {
    ...
    try {
      ...
      const response = await fetch(`/api/admin/engine/errors?${params}`, { headers: await authHeaders(), cache: "no-store" });
  ...
  async function update(nextStatus: string) {
    ...
    try {
      const response = await fetch("/api/admin/engine/errors", {
        ...
        body: JSON.stringify({ id: selected.id, status: nextStatus, notes, assigned_to: assignedTo || null }),
      });
      const body = await response.json();
  ...
  const totals = useMemo(() => trend.reduce((acc: Record<string, number>, item: Row) => {
    acc[item.severity] = (acc[item.severity] || 0) + Number(item.occurrence_count || 1);
    return acc;
  }, {}), [trend]);
  ...
  const dailyTrend = useMemo(() => {
    const days = Array.from({ length: 14 }, (_, offset) => {
      ...
      return { key: date.toISOString().slice(0, 10), label: date.toLocaleDateString(undefined, { month: "short", day: "numeric" }), count: 0 };
    });
  ...
  return <section className="space-y-4 rounded-[15px] border border-plum/10 bg-white p-5">
    <div className="flex flex-wrap items-start justify-between gap-3">
      <div><div className="flex items-center gap-2"><AlertTriangle className="text-magenta" size={21}/><h2 className="font-serif text-2xl text-plum">System Health / Error Monitoring</h2></div><p className="mt-1 text-xs text-ink/55">Sanitized operational failures are grouped by cause and release. Ordinary form validation is excluded.</p></div>
    ...
    <div className="grid gap-3 md:grid-cols-[140px_140px_180px_1fr_auto]">
      ...
      <label className="flex min-h-10 items-center gap-2 rounded-lg border px-3"><Search size={14}/><input value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") void load(); }} placeholder="Reference, route, or technical cause" className="min-w-0 flex-1 text-xs outline-none"/></label>
    ...
    <div className="grid gap-4 xl:grid-cols-[.9fr_1.1fr]">
      <div className="max-h-[560px] space-y-2 overflow-y-auto">
        {events.map((event) => <button key={event.id} type="button" onClick={() => selectEvent(event)} className={`w-full rounded-xl border p-4 text-left ${selected?.id === event.id ? "border-magenta bg-blush/30" : "border-plum/10"}`}><div className="flex items-center justify-between gap-3"><b className="text-xs text-plum">{event.feature} · {event.action}</b><span className={`rounded-full px-2 py-1 text-[9px] font-bold uppercase ${event.severity === "critical" ? "bg-red-100 text-red-800" : event.severity === "high" ? "bg-amber-100 text-amber-800" : "bg-blush text-plum"}`}>{event.severity}</span></div><p className="mt-2 line-clamp-2 text-xs text-ink/65">{event.technical_message}</p><p className="mt-2 text-[9px] text-ink/45">{event.reference} · {event.occurrence_count} occurrence{event.occurrence_count === 1 ? "" : "s"} · {new Date(event.last_occurred_at).toLocaleString()}</p></button>)}
      ...
      {selected ? <article className="min-w-0 rounded-xl border border-plum/10 bg-cream/35 p-4">
        <div className="flex items-center justify-between gap-3"><h3 className="font-serif text-xl text-plum">{selected.feature}</h3><span className="text-[10px] font-bold text-magenta">{selected.status}</span></div>
        <dl className="mt-3 grid gap-2 text-xs sm:grid-cols-2"><div><dt className="font-bold">Reference</dt><dd className="break-all text-ink/60">{selected.reference}</dd></div><div><dt className="font-bold">Environment / release</dt><dd className="text-ink/60">{selected.environment} · {selected.release}</dd></div><div><dt className="font-bold">Route / action</dt><dd className="break-all text-ink/60">{selected.route || "Scheduled/background"} · {selected.action}</dd></div><div><dt className="font-bold">Actor / salon</dt><dd className="break-all text-ink/60">{selected.actor_role || "unknown"} · {selected.salon_id || "not recorded"}{selected.salon_id ? <Link href={`/admin/salons?salon=${selected.salon_id}`} className="ml-2 font-bold text-magenta">Open salon</Link> : null}</dd></div></dl>
        <div className="mt-4 rounded-lg bg-ink p-3 text-[11px] leading-5 text-white"><b>Sanitized cause</b><pre className="mt-2 max-h-52 overflow-auto whitespace-pre-wrap break-words font-mono">{selected.technical_message}{selected.technical_stack ? `\n\n${selected.technical_stack}` : ""}</pre></div>
      ...
      </article> : <div className="grid min-h-72 place-items-center rounded-xl border border-dashed border-plum/15 text-xs text-ink/50">Select an event to inspect its sanitized technical details.</div>}
    </div>
