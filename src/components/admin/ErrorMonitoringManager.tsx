  ...
  return <section className="space-y-4 rounded-[15px] border border-plum/10 bg-white p-5">
    <div className="flex flex-wrap items-start justify-between gap-3">
      ...
      <div className="flex flex-wrap gap-2">
        <button type="button" disabled={Boolean(exporting)} onClick={() => void exportEvents("csv")} className="inline-flex min-h-10 items-center gap-2 rounded-lg border border-plum/15 px-4 text-xs font-bold text-plum disabled:cursor-wait gc-disabled-control"><Download size={14}/>{exporting === "csv" ? "Exporting…" : "Export CSV"}</button>
        <button type="button" disabled={Boolean(exporting)} onClick={() => void exportEvents("json")} className="inline-flex min-h-10 items-center gap-2 rounded-lg border border-plum/15 px-4 text-xs font-bold text-plum disabled:cursor-wait gc-disabled-control"><Download size={14}/>{exporting === "json" ? "Exporting…" : "Export JSON"}</button>
    ...
    <p className="text-[10px] gc-text-muted">Showing {events.length.toLocaleString()} of {total.toLocaleString()} matching incident{total === 1 ? "" : "s"}.</p>
    ...
    <div className={`grid gap-4 ${routeMode ? "" : "xl:grid-cols-[.9fr_1.1fr]"}`}>
      ...
      {selected ? <article className="min-w-0 rounded-xl border border-plum/10 bg-cream/35 p-4">
        ...
        <details className="mt-4 rounded-lg bg-ink p-3 text-[11px] leading-5 text-white"><summary className="cursor-pointer font-bold">Technical details</summary><div className="mt-2"><p className="break-all gc-text-on-dark">Reference {selected.reference} · {selected.environment} · {selected.release}</p><pre className="mt-2 max-h-52 overflow-auto whitespace-pre-wrap break-words font-mono">{selected.technical_message}{selected.technical_stack ? `\n\n${selected.technical_stack}` : ""}</pre></div></details>
        ...
        <div className="mt-3 flex flex-wrap gap-2"><button type="button" onClick={() => void update("Investigating")} className="rounded-lg border border-amber-300 px-3 py-2 text-[10px] font-bold gc-text-warning">Investigating</button><button type="button" onClick={() => void update("Resolved")} className="inline-flex items-center gap-1 rounded-lg bg-green-700 px-3 py-2 text-[10px] font-bold text-white"><CheckCircle2 size={13}/>Resolved</button><button type="button" onClick={() => void update("Ignored")} className="rounded-lg border px-3 py-2 text-[10px] font-bold">Ignore</button><button type="button" onClick={() => void update("Open")} className="rounded-lg border border-magenta px-3 py-2 text-[10px] font-bold text-magenta">Reopen</button></div>
