...
export default function AdminPromotionSectionWorkspace({ recordId }: { recordId: string }) {
  ...
  return <main className="min-h-screen bg-cream px-4 py-6 text-ink sm:px-8"><ActionToast message={notice} onDismiss={() => setNotice("")} /><div className="mx-auto max-w-[1500px]">
    ...
    <section className="mt-5 space-y-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="font-serif text-2xl text-plum">Saved cards</h2><p className="mt-1 text-xs text-ink/55">Changing layout never deletes these cards. Draft cards stay saved without becoming public.</p></div><button type="button" onClick={() => setSection((current) => current ? { ...current, cards: [...asRows(current.cards), newCard()] } : current)} className="inline-flex min-h-10 items-center gap-2 rounded-lg bg-magenta px-4 text-xs font-bold text-white"><Plus size={15}/>Add card</button></div>
      {cards.map((card, index) => <article key={card.id || index} className="rounded-2xl border border-plum/10 bg-white p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><span className="rounded-full bg-plum px-2 py-1 text-[9px] font-bold text-white">Position {index + 1}</span><p className={`mt-2 text-[10px] font-bold ${cardIssue(card).startsWith("Eligible") ? "text-green-700" : "text-red-700"}`}>{cardIssue(card)}</p></div><div className="flex gap-1"><button type="button" disabled={index === 0} onClick={() => moveCard(index, -1)} className="rounded-lg border p-2 disabled:opacity-30" aria-label="Move card up"><ArrowUp size={14}/></button><button type="button" disabled={index === cards.length - 1} onClick={() => moveCard(index, 1)} className="rounded-lg border p-2 disabled:opacity-30" aria-label="Move card down"><ArrowDown size={14}/></button><button type="button" onClick={() => setSection((current) => current ? { ...current, cards: asRows(current.cards).filter((_, itemIndex) => itemIndex !== index) } : current)} className="rounded-lg border border-red-200 p-2 text-red-700" aria-label="Remove card"><Trash2 size={14}/></button></div></div><div className="mt-4 grid gap-4 lg:grid-cols-2">
    ...
    <div className="sticky bottom-4 z-20 mt-6 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-plum/10 bg-white/95 p-4 shadow-xl backdrop-blur"><p className="text-[10px] leading-5 text-ink/55"><b className="block text-plum">Current layout: {layout}</b>Save draft keeps the existing public version. Publish replaces it only after database and public readback verification.</p><div className="flex flex-wrap gap-2"><button type="button" disabled={saving} onClick={() => void save("save_draft")} className="min-h-10 rounded-lg border border-plum/20 px-4 text-xs font-bold text-plum disabled:opacity-50">Save draft</button><button type="button" disabled={saving} onClick={() => void save("publish")} className="min-h-10 rounded-lg bg-magenta px-5 text-xs font-bold text-white disabled:opacity-50">{saving ? "Saving and verifying…" : "Publish"}</button><button type="button" disabled={saving} onClick={() => void save("unpublish")} className="min-h-10 rounded-lg border border-amber/50 px-4 text-xs font-bold text-plum disabled:opacity-50">Unpublish</button></div></div>
  </div></main>;
}
