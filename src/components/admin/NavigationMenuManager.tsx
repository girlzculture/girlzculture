    ...
    <section className="rounded-[15px] border border-plum/10 bg-white p-5">
      ...
      <div className="mt-5 grid gap-5 lg:grid-cols-[.9fr_1.1fr]">
        <div className="space-y-2">
          {loading ? <p className="text-xs text-ink/50">Loading navigation…</p> : visible.map((item,index) => <div key={item.id} className={`rounded-xl border p-3 ${selectedId === item.id ? "border-magenta bg-blush/20" : "border-plum/10"} ${item.archived_at ? "border-dashed bg-mist/60" : ""}`}>
            <button type="button" onClick={() => select(item)} className="w-full text-left"><span className="flex items-center justify-between gap-2"><b className="text-sm text-plum">{item.label}</b><span className="text-[9px] gc-text-muted">{item.href}</span></span><span className="mt-1 block text-[9px] gc-text-muted">{item.group_key} · order {item.sort_order}{item.archived_at ? " · archived" : item.is_enabled ? " · visible" : " · hidden"}</span></button>
            <div className="mt-2 flex gap-1"><button type="button" aria-label="Move earlier" disabled={busy||index===0||Boolean(item.archived_at)} onClick={() => void move(item,-1)} className="rounded border p-1 gc-disabled-control"><ArrowUp size={13}/></button><button type="button" aria-label="Move later" disabled={busy||index===visible.length-1||Boolean(item.archived_at)} onClick={() => void move(item,1)} className="rounded border p-1 gc-disabled-control"><ArrowDown size={13}/></button></div>
          </div>)}
        ...
        <form onSubmit={(event) => { event.preventDefault(); void submit(); }} className="rounded-xl border border-plum/10 p-4">
          ...
          <div className="mt-5 flex flex-wrap gap-2"><button type="submit" disabled={busy||Boolean(form.archived_at)} className="inline-flex min-h-10 items-center gap-2 rounded-lg bg-magenta px-5 text-xs font-bold text-white gc-disabled-control"><Save size={14}/>{selectedId?"Save item":"Add item"}</button>{selectedId ? (form.archived_at ? <button type="button" disabled={busy} onClick={() => void action(selectedId,"restore")} className="inline-flex min-h-10 items-center gap-2 rounded-lg border px-4 text-xs font-bold"><RotateCcw size={14}/>Restore</button> : <button type="button" disabled={busy} onClick={() => void action(selectedId,"archive")} className="inline-flex min-h-10 items-center gap-2 rounded-lg border border-red-200 px-4 text-xs font-bold gc-text-danger"><Archive size={14}/>Archive</button>) : null}</div>
