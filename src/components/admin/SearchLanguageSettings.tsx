...
export default function SearchLanguageSettings() {
  ...
  async function saveRule() {
    if (!selectedRule) return;
    if (selectedRule.ranking_boost === "") { setMessage("Enter a ranking boost between 0 and 100."); return; }
    setSaving(true); setMessage("");
    try {
  ...
  return <section className="rounded-[14px] border border-plum/10 bg-white/75 p-5 shadow-[0_8px_26px_rgba(13,17,20,.03)]">
    ...
    {!settings ? <div className="mt-5 flex items-center gap-2 text-xs text-ink/60"><RefreshCw size={15} className="animate-spin"/>Loading search controls...</div> : <>
      ...
      <button type="button" disabled={saving} onClick={() => void saveSettings()} className="mt-3 inline-flex min-h-10 items-center gap-2 rounded-lg border border-magenta px-4 text-xs font-bold text-magenta gc-disabled-control"><Save size={14}/>Save search defaults</button>
      <div className="mt-6 grid gap-4 xl:grid-cols-[280px_1fr]">
        ...
        {selectedRule ? <div className="rounded-xl border border-plum/10 bg-cream/35 p-4"><div className="flex items-center justify-between gap-3"><h3 className="font-serif text-lg text-plum">{selectedTarget?.name}</h3><label className="flex items-center gap-2 text-xs font-bold"><input type="checkbox" checked={selectedRule.is_active} onChange={(event) => updateRule({ is_active: event.target.checked })} className="accent-magenta"/>Rule active</label></div><div className="mt-3 grid gap-3 sm:grid-cols-2">{([['aliases','Aliases'],['keywords','Keywords'],['common_phrases','Common phrases'],['misspellings','Misspellings']] as const).map(([field,label]) => <label key={field} className="text-xs font-bold">{label}<textarea value={selectedRule[field].join(", ")} onChange={(event) => updateRule({ [field]: splitTerms(event.target.value) })} rows={3} className="mt-1 w-full rounded-lg border border-plum/15 bg-white p-3 font-normal"/><span className="mt-1 block text-[10px] font-normal text-ink/45">Separate entries with commas.</span></label>)}</div><label className="mt-3 block max-w-xs text-xs font-bold">Ranking boost<NumericInput min={0} max={100} decimalPlaces={2} value={selectedRule.ranking_boost} onValueChange={(value) => updateRule({ ranking_boost: value === "" ? "" : Number(value) })} className="mt-1 min-h-10 w-full rounded-lg border border-plum/15 bg-white px-3 font-normal"/></label><button type="button" disabled={saving} onClick={() => void saveRule()} className="mt-4 inline-flex min-h-11 items-center gap-2 rounded-lg bg-magenta px-5 text-xs font-bold text-white gc-disabled-control"><Save size={15}/>{saving ? "Saving..." : "Save search language"}</button></div> : <div className="grid min-h-56 place-items-center rounded-xl border border-dashed border-plum/20 bg-cream/40 p-8 text-center text-sm text-ink/55">Choose a real service or category to manage the phrases that find it.</div>}
      </div>
    </>}
