...
export default function BrandAppearanceManager() {
  ...
  async function action(
    kind: "save_position" | "review" | "publish" | "restore",
    ...
    try {
      ...
      setMessage(
        kind === "publish"
          ...
          : kind === "restore"
            ? `Version ${targetVersion} restored as the new live version.`
            : kind === "review"
              ? "Review recorded. This draft is ready to publish."
              : "Draft saved. Review the updated previews before publishing.",
      );
      await load(selected.asset_key);
    } catch (error) {
  ...
  return (
    <section className="rounded-[15px] border border-plum/10 bg-white p-5">
      <div className="flex items-start gap-3">
        ...
        <div><h3 className="font-serif text-2xl text-plum">Brand & Appearance</h3><p className="mt-1 max-w-3xl text-xs leading-5 text-ink/60">Upload platform identity assets, position each draft, preview responsive placements, record review, publish without a code deploy, and restore prior versions.</p></div>
      </div>
      <ThemeSurfacePreviews />
      <div className="mt-5 grid gap-5 xl:grid-cols-[260px_minmax(0,1fr)]">
        <nav className="space-y-2" aria-label="Brand placements">
          {assets.map((asset) => <button key={asset.asset_key} type="button" onClick={() => choose(asset)} className={`w-full rounded-xl border p-3 text-left ${asset.asset_key === selectedKey ? "border-magenta bg-blush/30" : "border-plum/10"}`}><span className="block text-xs font-bold text-plum">{asset.display_name}</span><span className="mt-1 block text-[9px] text-ink/50">{asset.published_version ? `Live v${asset.published_version}` : "Not published"}{asset.draft_url ? ` · ${asset.draft_reviewed_at ? "Reviewed draft" : "Draft needs review"}` : ""}</span></button>)}
        </nav>
        {selected ? <div className="min-w-0">
          <div className="rounded-xl bg-cream p-4 text-xs leading-5 text-ink/65"><b className="text-plum">{selected.guidance}</b><span className="mt-1 block">Minimum {selected.min_width_px} × {selected.min_height_px}px · Maximum {(selected.max_bytes / 1_048_576).toFixed(1)} MB · {selected.allowed_mime_types.join(", ")}</span>{selected.draft_width_px && selected.draft_height_px ? <span className="mt-1 block gc-text-success">Current draft: {selected.draft_width_px} × {selected.draft_height_px}px</span> : null}</div>
          {selected.placement_description ? <p className="mt-3 rounded-xl border border-plum/10 p-3 text-xs leading-5 text-ink/65"><b className="text-plum">Appears in:</b> {selected.placement_description}</p> : null}
          <form onSubmit={upload} className="mt-4 rounded-xl border border-dashed border-magenta/35 p-4">
            ...
            <button disabled={busy} className="mt-3 inline-flex min-h-10 items-center gap-2 rounded-lg bg-magenta px-4 text-xs font-bold text-white gc-disabled-control"><Upload size={15}/>Upload draft</button>
          </form>
          ...
          <p className="mt-4 rounded-xl border border-plum/10 bg-cream/45 p-3 text-xs text-ink/65"><b className="text-plum">Required workflow:</b> Save Draft → check desktop, tablet, and mobile previews → Review → Publish. Changing the crop or alt text clears the prior review.</p>
          <div className="mt-4 flex flex-wrap gap-2"><button type="button" disabled={busy || !previewUrl} onClick={() => void action("save_position")} className="inline-flex min-h-11 items-center gap-2 rounded-lg border border-magenta px-4 text-xs font-bold text-magenta gc-disabled-control"><Save size={15}/>Save draft</button><button type="button" disabled={busy || !selected.draft_url} onClick={() => void action("review")} className="min-h-11 rounded-lg border border-plum px-5 text-xs font-bold text-plum gc-disabled-control">{selected.draft_reviewed_at ? "Review again" : "Mark reviewed"}</button><button type="button" disabled={busy || !selected.draft_url || !selected.draft_reviewed_at} onClick={() => void action("publish")} className="min-h-11 rounded-lg bg-plum px-5 text-xs font-bold text-white gc-disabled-control">Publish this asset</button></div>
          {assetVersions.length ? <div className="mt-5"><h4 className="font-serif text-lg text-plum">Published history</h4><div className="mt-2 space-y-2">{assetVersions.map((version) => <div key={version.id} className="flex items-center justify-between gap-3 rounded-lg border border-plum/10 p-3 text-xs"><span><b>Version {version.version}</b><span className="ml-2 text-ink/45">{version.action} · {new Date(version.created_at).toLocaleString()}</span></span><button type="button" disabled={busy || version.version === selected.published_version} onClick={() => void action("restore",version.version)} className="inline-flex min-h-9 items-center gap-1 rounded-lg border px-3 text-[10px] font-bold text-plum gc-disabled-control"><RotateCcw size={13}/>Restore</button></div>)}</div></div> : null}
...
function ThemeSurfacePreviews() {
  ...
  return (
    <div className="mt-5 rounded-xl border border-plum/10 bg-cream/35 p-4">
      <div className="flex flex-wrap items-end justify-between gap-2">
        <div>
          <h4 className="font-serif text-lg text-plum">Published theme propagation</h4>
          <p className="mt-1 text-xs text-ink/60">These previews use the live Engine colors and fonts. Edit their individual controls above, then publish to update all surfaces.</p>
        </div>
        <span className="rounded-full bg-blush px-3 py-1 text-[10px] font-bold text-plum">Desktop · tablet · mobile</span>
      </div>
      <div className="mt-3 grid gap-2 sm:grid-cols-2 xl:grid-cols-5">
        {surfaces.map((surface) => (
          <div key={surface} className="gc-brand-card overflow-hidden rounded-lg border border-plum/10">
            <div className="gc-brand-header flex items-center justify-between border-b border-plum/10 px-3 py-2">
              <b className="font-serif text-plum">Girlz Culture</b>
              <span className="h-2 w-8 rounded-full bg-amber" />
            </div>
            <div className="p-3">
              <p className="font-serif text-base text-plum">{surface}</p>
              <p className="gc-brand-muted mt-1 text-[10px]">Readable supporting copy and status treatment.</p>
              <button type="button" tabIndex={-1} className="gc-brand-primary-action mt-3 rounded-md bg-magenta px-3 py-1.5 text-[10px] font-bold text-white">Primary action</button>
            </div>
          </div>
        ))}
      </div>
