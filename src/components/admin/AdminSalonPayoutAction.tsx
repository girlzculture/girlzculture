...
function Fact({label,value,strong=false}:{label:string;value:string;strong?:boolean}){return <div className={`rounded-xl p-3 ${strong?"bg-plum text-white":"bg-white text-plum"}`}><b className={`block text-[10px] uppercase tracking-wide ${strong ? "gc-text-on-dark-muted" : "gc-text-secondary"}`}>{label}</b><span className="mt-1 block font-serif text-xl">{value}</span></div>;}
