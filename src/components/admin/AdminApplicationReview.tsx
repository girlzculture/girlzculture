...
export default function AdminApplicationReview({ id }: { id: string }) {
  ...
  return <main className="min-h-screen bg-cream px-4 py-6 text-ink sm:px-8"><RoleSessionBoundary scope="admin" />
    <div className="mx-auto max-w-[1280px]"><Link href="/admin/submissions" className="inline-flex items-center gap-2 text-sm font-bold text-magenta"><ArrowLeft size={17}/>Back to submissions</Link>
      ...
      </section><aside className="space-y-5"><section className="rounded-[18px] border border-plum/10 bg-white p-5"><div className="flex items-center gap-3"><Building2 className="text-magenta"/><h2 className="font-serif text-2xl text-plum">Review decision</h2></div><p className="mt-3 text-sm leading-6 text-ink/60">Approval unlocks subscription checkout and onboarding. It does not publish the salon. Publication happens automatically only after payment and every marketplace setup gate are complete.</p><div className="mt-5 grid gap-3">{application.status === "Pending" ? <><button onClick={() => void decide("approve")} className="rounded-lg bg-magenta px-5 py-3 font-bold text-white">Approve application</button><button onClick={() => void decide("reject")} className="rounded-lg border border-magenta px-5 py-3 font-bold text-magenta">Reject with reason</button></> : application.status === "Approved" ? <div className="rounded-lg border border-amber/30 bg-amber/10 p-4 text-sm"><b className="text-plum">Application approved</b><p className="mt-1 text-ink/65">Waiting for the salon owner to activate a subscription and finish onboarding. The listing remains private until then.</p><Link href={`/admin/salons?salon=${String(application.salon_id || "")}`} className="mt-3 inline-flex font-bold text-magenta">View lifecycle and incomplete gates</Link></div> : <p className="rounded-lg bg-blush/30 p-3 text-sm">No pending application decision.</p>}</div></section>
