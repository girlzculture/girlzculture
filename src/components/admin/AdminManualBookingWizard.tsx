    ...
    <div className="space-y-5">
      ...
      <section className="rounded-2xl border border-plum/10 bg-white p-5">
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-2">
          ...
          <label className="text-xs font-bold">Service<select disabled={!salonId || loadingOptions} value={styleId} onChange={(event) => { setStyleId(event.target.value); setDate(""); setSlots([]); setSlotKey(""); setLoadingSlots(false); }} className="mt-1 min-h-11 w-full rounded-lg border border-plum/15 bg-white px-3 font-normal gc-disabled-control"><option value="">{loadingOptions ? "Loading services…" : "Choose service"}</option>{styles.map((item)=><option key={String(item.id)} value={String(item.id)}>{styleName(item)} · {money(item.base_price || item.price_display_min)}</option>)}</select></label>
        </div>
      </section>
      ...
      <section className="rounded-2xl border border-plum/10 bg-white p-5">
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-2">
          <label className="text-xs font-bold">Stylist<select disabled={!styleId} value={stylistId} onChange={(event) => { setStylistId(event.target.value); setSlotKey(""); }} className="mt-1 min-h-11 w-full rounded-lg border border-plum/15 bg-white px-3 font-normal gc-disabled-control"><option value="">Any available stylist</option>{stylists.map((item)=><option key={String(item.id)} value={String(item.id)}>{stylistName(item)}</option>)}</select></label>
          <label className="text-xs font-bold">Appointment date<input type="date" min={new Date().toISOString().slice(0,10)} disabled={!styleId} value={date} onChange={(event) => changeDate(event.target.value)} className="mt-1 min-h-11 w-full rounded-lg border border-plum/15 px-3 font-normal gc-disabled-control"/></label>
        </div>
        <div className="mt-4">
          ...
          <div className="mt-2 flex flex-wrap gap-2">{filteredSlots.map((slot) => {
            ...
            return <button key={key} type="button" onClick={() => setSlotKey(key)} className={`min-h-11 rounded-lg border px-4 text-xs font-bold ${active ? "border-magenta bg-magenta text-white" : "border-plum/15 bg-white text-plum"}`}>{String(slot.label || slot.value || "Time")}{!stylistId && (slot.stylistName || slot.stylist_name) ? <span className={`ml-1 font-normal ${active ? "gc-text-on-dark-muted" : "gc-text-secondary"}`}>· {String(slot.stylistName || slot.stylist_name)}</span> : null}</button>;
          })}</div>
        </div>
      </section>
      ...
      <section className="rounded-2xl border border-plum/10 bg-white p-5">
        ...
        <p className="mt-3 text-[10px] leading-5 gc-text-secondary">Platform Admin overrides do not require an internal-reason field. The selected override, administrator, time, appointment, and amounts are recorded automatically in the audit history.</p>
      </section>
      ...
      <section className="rounded-2xl border border-plum/10 bg-charcoal p-5 text-white">
        ...
        <dl className="mt-4 grid gap-3 text-xs sm:grid-cols-2 lg:grid-cols-3">
          <div><dt className="gc-text-on-dark">Customer</dt><dd className="mt-1 font-bold">{guestName || "Not selected"}</dd></div>
          <div><dt className="gc-text-on-dark">Salon</dt><dd className="mt-1 font-bold">{selectedSalonName}</dd></div>
          <div><dt className="gc-text-on-dark">Service</dt><dd className="mt-1 font-bold">{styleName(selectedStyle)}</dd></div>
          <div><dt className="gc-text-on-dark">Stylist</dt><dd className="mt-1 font-bold">{stylistName(selectedStylist || (selectedSlot ? { name: String(selectedSlot.stylistName || selectedSlot.stylist_name || "Any available stylist") } : null))}</dd></div>
          <div><dt className="gc-text-on-dark">Appointment</dt><dd className="mt-1 font-bold">{date && selectedSlot ? `${date} · ${String(selectedSlot.label || selectedSlot.value || "")}` : "Not selected"}</dd></div>
          <div><dt className="gc-text-on-dark">Service total</dt><dd className="mt-1 font-bold">{money(servicePrice)}</dd></div>
          <div><dt className="gc-text-on-dark">Standard deposit</dt><dd className="mt-1 font-bold">{depositPercentage}% · {money(calculatedDeposit)}</dd></div>
          <div><dt className="gc-text-on-dark">Selected handling</dt><dd className="mt-1 font-bold">{paymentMethods.find((item)=>item.value===paymentMethod)?.title}</dd></div>
        </dl>
        <button type="button" disabled={saving} onClick={() => void submit()} className="mt-5 inline-flex min-h-12 items-center gap-2 rounded-lg bg-magenta px-6 text-sm font-bold text-white gc-disabled-control">{saving ? "Preparing booking…" : <><Check size={17}/>Confirm and continue</>}</button>
      </section>

      {result?.payment_link ? <section className="rounded-2xl border border-emerald-200 bg-emerald-50 p-5"><div className="flex items-center gap-3"><Mail className="gc-text-success" size={20}/><div><h2 className="font-serif text-xl gc-text-success">Customer payment link ready</h2><p className="mt-1 text-xs gc-text-success">The appointment remains a temporary hold until Stripe confirms payment.</p></div></div><div className="mt-4 flex flex-col gap-2 sm:flex-row"><input readOnly value={String(result.payment_link)} className="min-h-11 min-w-0 flex-1 rounded-lg border border-emerald-200 bg-white px-3 text-xs"/><button type="button" onClick={() => void copyPaymentLink()} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-lg bg-emerald-700 px-5 text-xs font-bold text-white"><Clipboard size={15}/>Copy link</button><a href={String(result.payment_link)} target="_blank" rel="noreferrer" className="inline-flex min-h-11 items-center justify-center rounded-lg border border-emerald-700 px-5 text-xs font-bold gc-text-success">Open Stripe</a></div><p className="mt-3 text-[10px] gc-text-success">Intent {String(result.booking_intent_id || "")} · expires {String(result.expires_at || "")}</p></section> : null}
    </div>
  );
}
