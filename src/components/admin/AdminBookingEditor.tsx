...
export default function AdminBookingEditor({
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const action = String(submitter?.value || "update");
      const response = await fetch(`/api/admin/bookings/${bookingId}`, {
        method: "PATCH",
        headers: {
        ...
        body: JSON.stringify({
          ...
          stylist_id: form.get("stylist_id"),
          ...
          options: [
            ...
            form.get("alternative_1"),
            form.get("alternative_2"),
          ].filter(Boolean),
        }),
      });
      const body = await response.json();
        ...
        action === "cancel"
          ...
          : action === "propose_reschedule"
            ...
            : action === "correct_service_state"
              ? "Service state corrected with an immutable audit entry."
          ...
          <form onSubmit={submit} className="mt-6 space-y-4">
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <label className="text-sm font-semibold">
                ...
                <select
                  name="status"
                  defaultValue={String(booking.status || "Confirmed")}
                ...
                <small className="mt-1 block font-normal text-ink/55">
                  Use Correct Service State below. A reason and audit entry are
                  required.
                </small>
              </label>
              <label className="text-sm font-semibold">
                ...
                <select
                  name="style_id"
                  defaultValue={String(booking.style_id || "")}
                  className="mt-2 w-full rounded-lg border border-plum/15 bg-white p-3 font-normal"
                >
                  {styles.map((style) => (
              ...
              <label className="text-sm font-semibold">
                ...
                <select
                  name="stylist_id"
                  defaultValue={String(booking.stylist_id || "")}
                  className="mt-2 w-full rounded-lg border border-plum/15 bg-white p-3 font-normal"
                >
              ...
              <label className="text-sm font-semibold sm:col-span-2">
                <span className="flex items-center gap-2">
                  ...
                  Proposed or intervention time
                </span>
                <input
                  name="appointment_local"
                  type="datetime-local"
                  className="mt-2 w-full rounded-lg border border-plum/15 bg-white p-3 font-normal"
                />
                <small className="mt-1 block text-ink/60">
                  Proposals require customer approval. Save Admin Intervention
                  changes the booking immediately and records an audit entry.
                </small>
              </label>
              <Field
                name="alternative_1"
                label="Alternative proposed time"
                type="datetime-local"
                defaultValue=""
                required={false}
              />
            ...
            <label className="block text-sm font-semibold">
              Customer-facing proposal message
              <textarea
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <button
                name="action"
                value="propose_reschedule"
                disabled={busy}
                className="rounded-lg bg-plum px-5 py-3 font-bold text-white"
              >
                Propose to Customer
              </button>
              <button
                ...
                value="update"
                disabled={busy}
                className="rounded-lg bg-magenta px-5 py-3 font-bold text-white"
              >
                Save Admin Intervention
              </button>
              <button
                name="action"
                value="correct_service_state"
                disabled={busy}
                className="rounded-lg border border-plum px-5 py-3 font-bold text-plum"
              >
            ...
            {message ? (
              <p
                role="status"
                className="rounded-lg bg-blush/45 p-3 text-sm text-plum"
              >
                {message}
              </p>
            ) : null}
        ...
        <section className="mt-7 border-t border-plum/10 pt-5">
          ...
          {audit.length ? (
            audit.map((item) => (
              <p
                key={String(item.id)}
                className="border-b border-plum/10 py-3 text-sm"
              >
                <b className="capitalize">{String(item.action)}</b>
