  ...
  return <div className={className}>
    ...
    {selected && sourcePreview ? <section className="mt-4 rounded-[16px] border border-plum/15 bg-cream/55 p-4" aria-label={`Edit ${label}`}>
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_240px]">
        <div><div onPointerDown={beginCropDrag} onPointerMove={moveCrop} onPointerUp={endCropDrag} onPointerCancel={endCropDrag} className="relative mx-auto max-h-[520px] max-w-2xl touch-none overflow-hidden rounded-[13px] bg-ink/10" style={{ aspectRatio: aspect, backgroundImage: renderedPreview ? `url(${renderedPreview})` : "none", backgroundSize: "100% 100%", backgroundRepeat: "no-repeat" }}>{activeProfile.safeArea ? <div className="pointer-events-none absolute inset-[10%] rounded-lg border border-dashed border-white/80 shadow-[0_0_0_999px_rgba(13,17,20,.12)]" aria-hidden="true" /> : null}</div><p className="mt-2 text-center text-[10px] text-ink/55">Canonical {device} crop · drag with a mouse or finger. This exact rendered crop is uploaded.</p></div>
