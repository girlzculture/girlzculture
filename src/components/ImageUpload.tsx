...
export default function ImageUpload({ bucket, value, onChange, label, helperText, folder, multiple = false, maxFiles = 8, disabled = false, className, authScope = "customer", preset }: ImageUploadProps) {
  ...
  const animatedGif = selected?.type === "image/gif";
  ...
  async function upload() {
    ...
    try {
      ...
      const outputs = animatedGif
        ? [["desktop", selected] as const]
        : await Promise.all(DEVICES.map(async (target) => [target, await optimizeImageFile(selected, profileForRendition(profile, target), transforms[target])] as const));
      ...
      form.set("crop_metadata", JSON.stringify(animatedGif ? { version: 1, source: dimensions, mode: "animated_original", devicePreview: true } : { version: 1, source: dimensions, transforms }));
  ...
  return <div className={className}>
    <div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-sm font-bold uppercase tracking-[.16em] text-magenta">{label}</p>{helperText ? <p className="mt-1 text-xs leading-5 text-ink/65">{helperText}</p> : null}</div><ul className="space-y-0.5 text-[10px] font-semibold text-ink/55"><li>{profile.acceptedMimeTypes?.includes("image/gif") ? "JPG, PNG, or animated GIF" : "JPG or PNG"} · maximum original size 12 MB</li><li>Recommended {profile.minWidth}×{profile.minHeight}px or larger</li><li>{profile.acceptedMimeTypes?.includes("image/gif") ? "Images save device crops; GIF animation is preserved" : "Each device crop is saved as its own rendition"}</li></ul></div>
    <input ref={inputRef} type="file" accept={profile.acceptedMimeTypes?.join(",") || "image/jpeg,image/png"} onChange={(event) => { const file = event.target.files?.[0]; if (file) void prepare(file); }} className="sr-only" />
    ...
    {selected && sourcePreview ? <section className="mt-4 rounded-[16px] border border-plum/15 bg-cream/55 p-4" aria-label={`Edit ${label}`}>
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_240px]">
        <div><div onPointerDown={animatedGif ? undefined : beginCropDrag} onPointerMove={animatedGif ? undefined : moveCrop} onPointerUp={animatedGif ? undefined : endCropDrag} onPointerCancel={animatedGif ? undefined : endCropDrag} className="relative mx-auto max-h-[520px] max-w-2xl touch-none overflow-hidden rounded-[13px] bg-ink/10" style={{ aspectRatio: aspect, backgroundImage: renderedPreview ? `url(${renderedPreview})` : "none", backgroundSize: animatedGif ? "cover" : "100% 100%", backgroundPosition: "center", backgroundRepeat: "no-repeat" }}>{activeProfile.safeArea ? <div className="pointer-events-none absolute inset-[10%] rounded-lg border border-dashed border-white/80 shadow-[0_0_0_999px_rgba(13,17,20,.12)]" aria-hidden="true" /> : null}</div><p className="mt-2 text-center text-[10px] text-ink/55">{animatedGif ? `Animated GIF preview in the ${device} card frame. Animation and original pixels are preserved.` : `Canonical ${device} crop · drag with a mouse or finger. This exact rendered crop is uploaded.`}</p></div>
        {animatedGif ? <div className="rounded-xl border border-amber/30 bg-white p-4 text-xs leading-5 text-ink/65">Animated GIFs are previewed for desktop, tablet, and mobile without flattening their frames. Static JPG and PNG files provide independent crop controls for each device.</div> : <div><label className="block text-[10px] font-bold">Zoom<input aria-label={`${device} image zoom`} type="range" min="1" max="3" step="0.05" value={transform.zoom || 1} onChange={(event) => setTransform((current) => ({ ...current, zoom: Number(event.target.value) }))} className="mt-1 w-full accent-magenta" /></label>
      ...
      <div className="mt-4 flex flex-wrap justify-end gap-2"><button type="button" onClick={resetEditor} className="min-h-11 rounded-lg border border-plum/15 px-4 text-xs font-bold text-plum">Cancel</button><button type="button" onClick={() => { resetEditor(); inputRef.current?.click(); }} className="min-h-11 rounded-lg border border-magenta px-4 text-xs font-bold text-magenta">Replace</button><button type="button" disabled={busy} onClick={() => void upload()} className="inline-flex min-h-11 items-center gap-2 rounded-lg bg-magenta px-5 text-xs font-bold text-white disabled:opacity-60"><UploadCloud size={16}/>{busy ? "Uploading..." : animatedGif ? "Save animated GIF" : "Save all crops"}</button></div>
    </section> : null}
