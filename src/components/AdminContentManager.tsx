...
import AdminMarketWorkspaces from "@/components/admin/AdminMarketWorkspaces";
...
export default function AdminContentManager({
  ...
  if (!acceptanceAccessToken && !initialRecordId) {
    return <div className="space-y-6"><AdminMarketWorkspaces scope="content" pages={pages}/><ContentManagerLanding pages={pages} posts={posts} publicationByPage={publicationByPage} /></div>;
  }
...
function SectionEditor({ section, index, sectionCount, linkTargets, update, remove, move, allowRemove = true }: { section: Row; index: number; sectionCount:number; linkTargets: Row[]; update: (section: Row) => void; remove: () => void;move:(direction:-1|1)=>void; allowRemove?: boolean }) {
  ...
  const [areaFilter, setAreaFilter] = useState(areaParams.get("market") || "all");
  ...
  return <div className="rounded-lg border border-plum/10 bg-blush/25 p-4">
    ...
    {cards.length ? <label className="mt-4 block text-sm font-bold">Filter cards by audience<select value={areaFilter} onChange={event => setAreaFilter(event.target.value)} className="ml-3 min-h-11 max-w-full border bg-white px-3"><option value="all">All audiences</option><option value="global">Global cards only</option>{linkTargets.filter(target => target.type === "Market").map(target => <option key={target.id} value={target.id}>{target.label}</option>)}</select><span className="mt-1 block text-sm font-normal">This view filter never changes hidden cards or their publication settings.</span></label> : null}
    {cards.length ? <div className="mt-4 grid items-start gap-4 xl:grid-cols-2">{cards.map((card, cardIndex) => ({ card, cardIndex })).filter(({ card }) => areaFilter === "all" || (areaFilter === "global" ? !card.market_id && !["salon", "campaign"].includes(String(card.association_type)) : card.market_id === areaFilter)).map(({ card, cardIndex }) => <article key={card.id || cardIndex} className="self-start rounded-xl border border-plum/10 bg-white p-4">
