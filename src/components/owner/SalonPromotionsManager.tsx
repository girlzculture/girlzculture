...
export default function SalonPromotionsManager({ promotions, styles, products, setPromotions, saveRecord, removeRecord, recordId = "" }: Props) {
  ...
  return <>
    ...
    {recordId ? <section id="promotion-editor" className="mt-5 rounded-[15px] border border-plum/10 bg-white px-3 py-5 max-sm:-mx-3 sm:p-5">
      ...
      <form key={editing?.id || "new"} onSubmit={submit} className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <Label text="Offer type" className="min-w-0 xl:col-span-2"><select name="promotion_type" defaultValue={String(editing?.promotion_type || "percentage")} className={`${inputClass} max-sm:px-2`}><option value="percentage">Percentage discount</option><option value="fixed">Fixed discount</option><option value="free_addon">Free eligible add-on</option><option value="free_service">Free eligible service</option><option value="descriptive">Descriptive offer</option></select></Label>
