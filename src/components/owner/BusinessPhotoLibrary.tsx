...
import { Camera, ImagePlus, Images, Pencil, Star, Upload, X } from "lucide-react";
import ImageUpload from "@/components/ImageUpload";
import SafeImage from "@/components/site/SafeImage";
import { useI18n } from "@/components/i18n/LocaleProvider";
import { getSessionForScope } from "@/lib/supabase";
import { OwnerActionError, readOwnerResponse } from "@/lib/ownerActionError";
import { BUSINESS_PHOTO_CATEGORIES, defaultPhotoDetails, publicGalleryPhotos, type BusinessPhotoCategory, type BusinessPhotoDetails, type BusinessPhotoMetadata } from "@/lib/businessPhotoMetadata";

type BusinessMedia = { id: string; name?: string; slug?: string; gallery_photos?: string[]; photo_metadata?: BusinessPhotoMetadata; cover_photo_url?: string; logo_url?: string };
export default function BusinessPhotoLibrary({ salon, onSaved }: { salon: BusinessMedia; onSaved: (patch: Partial<BusinessMedia>) => void }) {
  ...
  return <div className="space-y-5">
    <header className="flex flex-wrap items-end justify-between gap-3"><div><h1 className="font-serif text-3xl text-ink sm:text-4xl">{t("Photos")}</h1><p className="mt-2 text-sm text-muted">{t("Showcase your work. Attract more clients. Grow your business.")}</p></div><div className="flex flex-wrap gap-2"><Link href={`/salon/${salon.slug}`} className="flex min-h-11 items-center rounded-xl border border-border px-4 text-sm font-semibold">{t("View public gallery")}</Link><button onClick={openUpload} className="flex min-h-11 items-center gap-2 rounded-xl bg-primary px-4 text-sm font-semibold text-white"><Upload size={17}/>{t("Upload Photos")}</button></div></header>
    <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">{[["Gallery photos", gallery.length, Images], ["Cover photo", salon.cover_photo_url ? 1 : 0, Camera], ["Salon logo", salon.logo_url ? 1 : 0, ImagePlus], ["Featured photos", gallery.filter(url => metadata[url]?.featured).length, Star]].map(([label,value,Icon]) => { const MetricIcon = Icon as typeof Images; return <div key={String(label)} className="flex items-center justify-between gap-3 rounded-xl border border-border bg-white p-4"><div><p className="text-xs text-muted">{t(String(label))}</p><p className="mt-2 font-serif text-2xl">{formatNumber(Number(value))}</p></div><MetricIcon size={20} className="shrink-0 text-primary"/></div>; })}</div>
    ...
    <dialog ref={dialog} onClose={() => setSelected(null)} aria-labelledby="photo-details-title" className="m-auto max-h-[90dvh] w-[min(95vw,560px)] overflow-y-auto rounded-2xl border border-border bg-white p-5 text-ink backdrop:bg-black/30"><form onSubmit={async event => { event.preventDefault(); if (!selected || busy) return; setBusy(true); setNotice(""); setReference(""); try { await persist(selected, draft, expected); setSelected(null); } catch (error) { fail(error); } finally { setBusy(false); } }} className="space-y-4"><header className="flex items-center justify-between gap-3"><h2 id="photo-details-title" className="font-serif text-2xl">{t("Edit photo details")}</h2><button type="button" onClick={() => setSelected(null)} aria-label={t("Close")} className="grid h-11 w-11 place-items-center"><X size={20}/></button></header>{notice ? <p role="alert" className="text-sm text-red-700">{t(notice)} {reference}</p> : null}<label className="block text-sm font-semibold">{t("Category")}<select value={draft.category} onChange={event => setDraft(value => ({ ...value, category: event.target.value as BusinessPhotoCategory }))} className="mt-2 min-h-11 w-full rounded-lg border border-border p-2">{Object.entries(BUSINESS_PHOTO_CATEGORIES).map(([value,label]) => <option key={value} value={value}>{t(label)}</option>)}</select></label><label className="block text-sm font-semibold">{t("Title")}<input value={draft.title} maxLength={120} onChange={event => setDraft(value => ({ ...value, title: event.target.value }))} className="mt-2 min-h-11 w-full rounded-lg border border-border p-3"/></label><label className="block text-sm font-semibold">{t("Caption")}<textarea rows={4} value={draft.caption} maxLength={1000} onChange={event => setDraft(value => ({ ...value, caption: event.target.value }))} className="mt-2 w-full rounded-lg border border-border p-3"/></label><label className="flex min-h-11 items-center gap-3 text-sm"><input type="checkbox" checked={draft.featured} onChange={event => setDraft(value => ({ ...value, featured: event.target.checked }))}/>{t("Featured photo")}</label><button disabled={busy} className="min-h-11 w-full rounded-xl bg-primary px-4 text-sm font-semibold text-white disabled:opacity-60">{t(busy ? "Saving…" : "Save photo details")}</button></form></dialog>
  </div>;
}
