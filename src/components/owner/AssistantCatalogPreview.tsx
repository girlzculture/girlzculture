...
export default function AssistantCatalogPreview({value,locale}:{value:Row;locale:string}){
 ...
 function display(key:string,item:unknown):string{
  ...
  if(item==null)return key==="availability"?copy.business_hours:copy.unset;
  if(typeof item==="boolean")return item?copy.yes:copy.no;
  if(typeof item==="number")return ["base_price","price_display_min","price_display_max","price","sale_price","shipping_price","price_add"].includes(key)?money.format(item):key==="discount_value"?values.promotion_type==="percentage"?number.format(item)+"%":money.format(item):number.format(item);
  if(Array.isArray(item))return item.length?item.map(value=>display(key,value)).join("; "):copy.empty;
  if(typeof item==="object")return Object.entries(item as Row).map(([field,value])=>`${copy[field]||field}: ${display(field,value)}`).join(" · ");
  return ["status","product_status","promotion_type","target_scope"].includes(key)?copy[String(item)]||String(item):String(item);
 }
 return <section data-no-translate aria-label={copy.review} className="space-y-3"><h4 className="font-semibold">{copy.review}</h4><p className="break-words">{String(values.name||values.title||"")}</p>{value.time_zone?<p className="text-sm">{copy.time_zone}: {String(value.time_zone)}</p>:null}<dl className="space-y-2">{Object.entries(changes).filter(([key])=>key!=="master_style_id").map(([key,item])=><div key={key}><dt className="text-xs font-semibold">{copy[key]||key}</dt><dd className="mt-1 break-words whitespace-pre-wrap text-sm">{display(key,item)}</dd></div>)}</dl></section>;
}
