...
export function Facts({ value, name = "", group = "", depth = 0, timeZone = "America/New_York" }: { value: unknown; name?: string; group?: string; depth?: number; timeZone?: string }) {
  ...
  if (["status", "product_status", "moderation_status", "source", "booking_origin", "payment_status", "customer_policy_acceptance"].includes(name)) return <span>{t(String(value))}</span>;
...
export default function GcAssistant({ children }: { children?: React.ReactNode } = {}) {
  ...
  async function submit(tool?: string, setup = false) {
    ...
    try {
      const result = await call(tool ? { action: "tool", request_id: id, tool, args: tool === "get_services_and_prices" ? { query: "" } : {} } : { action: "plan", request_id: id, text: message, conversation: turns.flatMap(turn => [...(turn.text ? [{ role: "user", text: turn.text }] : []), ...(turn.clarification ? [{ role: "assistant", text: turn.clarification }] : [])]).slice(-6), previous_request_ids: turns.filter(turn => turn.request).slice(-6).map(turn => turn.request!.id) }, generation);
  ...
  return <AssistantOpenContext.Provider value={button => { launcher.current = button; dialog.current?.showModal(); }}>{children}
    <dialog ref={dialog} aria-labelledby="gc-assistant-title" onClose={() => { setDictationSession(value => value + 1); launcher.current?.focus(); }} className="fixed inset-auto bottom-0 right-0 m-0 h-[85dvh] max-h-[900px] w-full max-w-xl overflow-hidden rounded-t-2xl border border-plum/20 bg-cream p-0 text-ink backdrop:bg-black/40 sm:bottom-4 sm:right-4 sm:w-[calc(100%-2rem)] sm:rounded-2xl">
      <div className="flex h-full flex-col"><header className="flex shrink-0 items-center justify-between border-b p-4"><h2 id="gc-assistant-title" className="font-serif text-2xl" data-no-translate>GC Assistant</h2><button onClick={() => dialog.current?.close()} className="grid h-11 w-11 place-items-center rounded-full border" aria-label={t("Close GC Assistant")}><X aria-hidden size={20}/></button></header>
        ...
        </div><form className="shrink-0 space-y-2 border-t bg-cream p-4" onSubmit={event => { event.preventDefault(); void submit(); }}><label htmlFor="gc-assistant-input" className="text-sm font-semibold">{t("What would you like help with?")}</label><textarea id="gc-assistant-input" data-no-translate maxLength={2400} value={text} onChange={event => setText(event.target.value)} className="block max-h-36 min-h-20 w-full rounded-xl border bg-white p-3 text-sm"/><AssistantDictation key={`${locale}:${dictationSession}`} sessionKey={dictationSession} disabled={busy} onTranscript={transcript => setText(current => `${current}${current ? " " : ""}${transcript}`.slice(0, 2400))}/><button disabled={busy || !text.trim()} className="min-h-11 rounded-full bg-plum px-6 text-sm text-white gc-disabled-control">{t("Ask GC Assistant")}</button></form>
      </div>
