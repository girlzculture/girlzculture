...
export default function BusinessPolicies() {
  ...
  useEffect(() => { let active = true; void call().then(result => { if (active) { setRevisions(result.revisions); const current = result.revisions.find((item: Revision) => item.id === result.current); if (current) setPolicy({ ...POLICY_DEFAULTS, ...current.policy }); } }).catch(error => { if (active) { setNotice("Business policies are temporarily unavailable."); setReference(error instanceof OwnerActionError ? error.reference : ""); } }); return () => { active = false; }; }, [call]);
