...
export default function BusinessPolicies({ canManageDeposits = false }: { canManageDeposits?: boolean }) {
  ...
  return <section className="min-w-0 space-y-4"><header className="flex items-start gap-3"><Link href="/salon/dashboard/my-page" aria-label={t("Back to My Page")} className="inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-lg border border-border"><ArrowLeft aria-hidden size={20}/></Link><div className="min-w-0"><h1 className="font-serif text-2xl text-plum sm:text-3xl">{t("Your Business Policies")}</h1><p className="mt-1 text-sm">{t("Set clear expectations for appointments. Changes apply to future bookings after you publish.")}</p>{canManageDeposits ? <a href="#booking-deposits" className="inline-flex min-h-11 items-center text-sm font-semibold text-primary underline">{t("Booking deposits")}</a> : null}</div></header>
    ...
    {canManageDeposits ? <div id="booking-deposits" className="scroll-mt-40"><BusinessDepositSettings /></div> : null}
