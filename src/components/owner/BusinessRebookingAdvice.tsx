...
function AdvicePanel({ canReviewUpdates }: { canReviewUpdates: boolean }) {
 ...
 return <section id="returning-clients" aria-labelledby="returning-clients-title" aria-busy={busy} className="mt-5 scroll-mt-24 space-y-3 rounded-xl border border-border bg-surface p-4 sm:p-5" data-no-translate>
  <div className="flex flex-wrap items-start justify-between gap-3"><h2 id="returning-clients-title" className="font-serif text-xl font-bold">{copy("title")}</h2><button type="button" className={button} disabled={busy} onClick={() => setRefresh(value => value + 1)}>{copy("refresh")}</button></div>
  ...
  {data && <>
   {data.scope === "assigned_professional" && <p className="text-sm font-medium">{copy("scope")}</p>}
   {!data.available ? <p role="status" className="rounded-lg bg-subtle p-3 text-sm">{copy("unavailable")}</p> : <>
    <p className="text-sm font-semibold">{data.completed_visit_count === 0 ? copy("noHistory") : data.absent_count === 1 ? copy("countOne") : data.absent_count ? copy("count", { count: formatNumber(data.absent_count) }) : copy("empty")}</p>
   ...
   <p className="text-xs text-muted">{copy("contact")}</p>
   <details className="rounded-lg border border-border px-3">
    <summary className="min-h-11 cursor-pointer content-center text-sm font-semibold text-primary">{copy("method")}</summary>
    <div className="space-y-3 pb-3 text-sm text-muted">
     <p>{copy("intro")}</p><p>{copy("period", { from: day(data.from), to: day(data.through), zone: data.time_zone, time: new Intl.DateTimeFormat(locale, { dateStyle: "medium", timeStyle: "short", timeZone: data.time_zone }).format(new Date(data.as_of)) })}</p>
     <p>{copy("definition")}</p><p>{copy("dateBasis")}</p><p>{copy("identity")}</p>
     {data.unlinked_guest_visits ? <p>{copy("unlinked", { count: formatNumber(data.unlinked_guest_visits) })}</p> : null}
     <p>{copy("consent")}</p>
    </div>
   </details>
  </>}
