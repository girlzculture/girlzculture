...
export function StructuredStylistsEditor({ c, recordId = "" }: { c: Context; recordId?: string }) {
  ...
  return <>
    {!recordId ? <EditorTitle compactLandscape title={businessLabels(c.salon.business_type).team || "Team"} subtitle="Manage your team with consistent specialties drawn from the platform style list." action={<button type="button" onClick={startNewStylist} className="min-h-11 rounded-[8px] bg-magenta px-6 py-3 text-xs font-bold text-white"><Plus className="mr-1 inline" size={16} />Add Stylist</button>} /> : null}
