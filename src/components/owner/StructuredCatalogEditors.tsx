...
type MasterStyle = Record<string, any> & { id: string; name: string; category?: string; category_id?: string; service_category?: { id?: string; name?: string; slug?: string } | null };
...
export function StructuredStylesEditor({ c }: { c: Context }) {
  ...
  useEffect(() => {
    let live = true;
    supabase.from("master_styles").select("*,service_category:service_categories(id,name,slug)").eq("is_active", true).order("sort_order").order("name").then(({ data, error }) => {
      if (!live) return;
  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    if (isBraiding && addons.some((item) => item.label === "Other: ")) { c.setNotice("Enter a name for each Other add-on."); return; }
    setSaving(true);
  ...
  return <>
    ...
    <div className="grid gap-4 xl:grid-cols-[.7fr_1.3fr]">
      ...
      <form key={active?.id || "new"} onSubmit={save}><EditorPanel><div className="flex items-center justify-between"><h2 className="font-serif text-xl text-plum">{active ? "Edit Service" : "Add Service"}</h2><span className="text-[9px] font-bold uppercase text-green-700">Category-aware</span></div>
        <div className="mt-4 grid gap-4 sm:grid-cols-2"><SelectField label="Service name" value={masterId} onChange={setMasterId} options={masters.filter((master) => master.id && master.name).map((master) => ({ value: String(master.id), label: `${master.service_category?.name || "Service"} · ${master.name}` }))} placeholder="Choose from managed catalog" /><ReadOnlyField label="Top-level category" value={chosenMaster?.service_category?.name || "Select a service"} /><ReadOnlyField label="Service group" value={chosenMaster?.category || active?.category || "General"} /><label className="sm:col-span-2 text-[10px] font-bold">Description<textarea value={description} onChange={(event) => setDescription(event.target.value.slice(0, 500))} rows={3} className="mt-1 w-full rounded-[7px] border border-plum/15 p-3 font-normal" /></label><NumberField label="Duration minimum (hours)" value={durationMin} onChange={setDurationMin} step="0.25" /><NumberField label="Duration maximum (hours)" value={durationMax} onChange={setDurationMax} step="0.25" /><MoneyInput label="Base price" value={basePrice} onChange={setBasePrice} /><MoneyInput label="Maximum displayed price" value={maxPrice} onChange={setMaxPrice} /><SelectField label="Cleanup buffer" value={String(bufferMinutes)} onChange={(value) => setBufferMinutes(Number(value))} options={[0,15,30,45,60].map((value) => ({ value: String(value), label: `${value} minutes` }))} />{isBraiding ? <><MoneyInput label="Customer brings hair: price reduction" value={ownMaterialPriceReduction} onChange={setOwnMaterialPriceReduction} /><NumberField label="Customer brings hair: minutes reduced" value={ownMaterialDurationReduction} onChange={setOwnMaterialDurationReduction} step="5" /></> : null}</div>
        {isBraiding ? <><div className="mt-5 grid gap-4 lg:grid-cols-2"><OptionEditor title="Size Options" options={SIZE_OPTIONS} rows={sizes} setRows={setSizes} /><OptionEditor title="Length Options" options={LENGTH_OPTIONS} rows={lengths} setRows={setLengths} /><OptionEditor title="Add-ons" options={ADD_ON_OPTIONS} rows={addons} setRows={setAddons} allowOther />
        ...
        <section className="mt-4 rounded-[11px] border border-plum/10 p-4"><h3 className="font-serif text-lg text-plum">What’s Included</h3><div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{INCLUDED_ITEM_OPTIONS.map((item) => <label key={item} className="flex items-center gap-2 text-xs"><input type="checkbox" checked={included.includes(item)} onChange={() => setIncluded((current) => current.includes(item) ? current.filter((value) => value !== item) : [...current, item])} className="accent-magenta" />{item}</label>)}</div></section></> : <section className="mt-5 rounded-[11px] border border-plum/10 bg-blush/25 p-4 text-xs leading-5 text-plum">This category uses the generic option-group model. Category-specific owner controls can be enabled when that category is launched; the shared catalog and checkout already support them.</section>}
