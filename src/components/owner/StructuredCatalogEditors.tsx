...
type Context = {
  ...
  saveRecord: (table: string, values: Record<string, unknown>, id?: string, throwOnFailure?: boolean) => Promise<any>;
...
function MoneyInput({ value, onChange, label = "Price" }: { value: NumericValue; onChange: (value: NumericValue) => void; label?: string }) {
  return <label className="text-sm font-bold">{label}<span className="mt-1 flex min-h-11 items-center rounded-[7px] border border-plum/15 bg-white px-3"><span className="mr-1 text-ink/45">$</span><NumericInput min={0} max={10000} decimalPlaces={2} value={value} onValueChange={onChange} className="min-w-0 flex-1 outline-none" /></span></label>;
}
...
function OptionEditor({ title, options, rows, setRows, allowOther = false }: { title: string; options: readonly string[]; rows: OptionRow[]; setRows: React.Dispatch<React.SetStateAction<OptionRow[]>>; allowOther?: boolean }) {
  ...
  return <section className="rounded-[11px] border border-plum/10 bg-cream/35 p-4">
    <div className="flex items-center justify-between gap-3"><h3 className="font-serif text-lg text-plum">{title}</h3><button type="button" onClick={add} className="flex items-center gap-1 text-sm font-bold text-magenta"><Plus size={13} />Add another</button></div>
    <div className="mt-3 space-y-2">{rows.map((row, index) => {
      ...
      return <div key={`${row.label}-${index}`} className="grid grid-cols-[1fr_105px_28px] items-end gap-2">
        <label className="text-sm font-bold">Option<select value={other ? "Other" : row.label} onChange={(event) => setRows((current) => current.map((item, rowIndex) => rowIndex === index ? { ...item, label: event.target.value === "Other" ? "Other: " : event.target.value } : item))} className="mt-1 min-h-11 w-full rounded-[7px] border border-plum/15 bg-white px-2 font-normal">{options.map((option) => <option key={option}>{option}</option>)}</select></label>
        ...
        {other ? <label className="col-span-3 text-sm font-bold">Other add-on name<input value={row.label.slice(7)} maxLength={80} onChange={(event) => setRows((current) => current.map((item, rowIndex) => rowIndex === index ? { ...item, label: `Other: ${event.target.value}` } : item))} className="mt-1 min-h-11 w-full rounded-[7px] border border-plum/15 bg-white px-3 font-normal" /></label> : null}
      </div>;
    })}{!rows.length ? <p className="rounded-[8px] border border-dashed border-plum/15 p-3 text-center text-sm text-ink/45">No options selected.</p> : null}</div>
  </section>;
}
...
function ManagedAddonEditor({ options, rows, setRows }: { options: string[]; rows: OptionRow[]; setRows: React.Dispatch<React.SetStateAction<OptionRow[]>> }) {
  return <section className="rounded-[11px] border border-plum/10 bg-cream/35 p-4"><h3 className="font-serif text-lg text-plum">Add-ons</h3><p className="mt-1 text-sm text-ink/55">Choose from the platform catalog, then set your price.</p><div className="mt-3 space-y-2">{options.map((option) => { const selected=rows.find((row)=>row.label===option); return <div key={option} className="grid grid-cols-[1fr_115px] items-center gap-3 rounded-[8px] border border-plum/10 bg-white p-2"><label className="flex items-center gap-2 text-xs"><input type="checkbox" checked={Boolean(selected)} onChange={(event)=>setRows((current)=>event.target.checked?[...current,{label:option,price_add:""}]:current.filter((row)=>row.label!==option))} className="accent-magenta"/>{option}</label>{selected?<MoneyInput value={selected.price_add} onChange={(value)=>setRows((current)=>current.map((row)=>row.label===option?{...row,price_add:value}:row))}/>:<span className="text-right text-sm text-ink/40">Not offered</span>}</div>;})}{!options.length?<Empty text="No active add-ons are available for this category."/>:null}</div></section>;
}

export function StructuredStylesEditor({ c, recordId = "" }: { c: Context; recordId?: string }) {
  const { t, translateSource, formatNumber } = useI18n();
  ...
  const bufferEdited = useRef(false);
  ...
  const serviceCategory = searchParams.get("category") || "all";
  const serviceGroup = searchParams.get("group") || "all";
  const serviceStatus = searchParams.get("status") || "all";
  ...
  useLayoutEffect(() => {
    let live = true;
    // Initialize before the first interactive paint. A later Engine default
    // response must never reset an existing record or an in-progress draft.
    bufferEdited.current = false;
  ...
  useEffect(() => {
    if (!active?.id && !bufferEdited.current) setBufferMinutes(engineOptions.defaultBuffer);
  }, [active?.id, engineOptions.defaultBuffer]);
  ...
  const visibleStyles = sortCatalogRecords(c.styles, { preserveSourceOrder: true }).filter((style) => {
  ...
  return <>
    {!recordId ? <EditorTitle title={businessLabels(c.salon.business_type).services} subtitle="Use category-aware service options so customers can compare and book accurately." action={<Link href={`/salon/dashboard/styles/new${listParams.toString() ? `?${listParams}` : ""}`} className="rounded-[8px] bg-magenta px-6 py-3 text-xs font-bold text-white"><Plus className="mr-1 inline" size={16} />Add Service</Link>} /> : null}
    {!recordId ? <ServicesWorkspace salon={c.salon} styles={c.styles} visible={visibleStyles} bookings={c.bookings} canReadBookings={c.access === null || c.access.bookings === true} categories={categories} groups={groups} query={serviceQuery} category={serviceCategory} group={serviceGroup} status={serviceStatus} setQuery={value=>setFilter("q",value)} setCategory={value=>setFilter("category",value)} setGroup={value=>setFilter("group",value)} setStatus={value=>setFilter("status",value)} listParams={listParams} save={async(id,patch)=>{const saved=await c.saveRecord("styles",patch,id);if(!saved)return false;if(saved.id!==id||saved.salon_id!==c.salon.id){c.setNotice("The saved business record could not be verified.");return false;}c.setStyles(rows=>rows.map(row=>row.id===id?saved:row));return true;}}/> : null}
    {!recordId ? <details className="my-5 rounded-xl border border-border bg-white p-4"><summary className="min-h-11 cursor-pointer content-center text-sm font-semibold text-primary">Import or export services</summary><SalonSpreadsheetPanel kind="services" onImported={records=>{c.setStyles(records);if(c.selectedStyle&&!records.some(record=>record.id===c.selectedStyle))c.setSelectedStyle(null);}}/></details> : null}
    <div className="block">
      <MobileRecordEditor open={Boolean(recordId)} title={active ? `Edit ${active.name || "service"}` : "Add service"} onClose={() => router.push(stylesListHref)}><form key={active?.id || "new"} onSubmit={save}><OwnerDetailHeader hideOnMobile title={active ? `Edit ${active.name || "service"}` : "Add service"} subtitle="Keep pricing, duration, options, inclusions, and service media together." fallbackHref={stylesListHref} status={active ? "Published service" : "New service"}/><EditorPanel><div className="flex items-center justify-between"><h2 className="font-serif text-xl text-plum">Service details</h2><span className="text-xs font-bold uppercase gc-text-success">Category-aware</span></div>
        <div className="mt-4 grid gap-4 sm:grid-cols-2"><SelectField label="Top-level category" value={categoryId} onChange={(value)=>{setCategoryId(value);setGroupId("");setMasterId("");setDisplayName("");setAddons([]);}} options={categories.map((category)=>({value:String(category.id),label:String(category.name)}))} placeholder="Choose category" /><SelectField label="Service group" value={groupId} onChange={(value)=>{setGroupId(value);setMasterId("");setDisplayName(String(groups.find((group)=>group.id===value)?.name||""));}} options={availableGroups.map((group)=>({value:String(group.id),label:String(group.name)}))} placeholder="Choose service group" /><SelectField required={false} label="Specific service name (optional)" value={masterId} onChange={(value)=>{setMasterId(value);const managed=masters.find((master)=>master.id===value);if(managed)setDisplayName(managed.name);}} options={availableServices.map((master)=>({value:String(master.id),label:master.name}))} placeholder="Sell the whole group" /><label className="text-sm font-bold">{t("services.customer_name_label", "Service name customers will see")}<input required value={displayName} onChange={(event)=>setDisplayName(event.target.value.slice(0,120))} disabled={Boolean(chosenMaster)} className="mt-1 min-h-11 w-full rounded-[7px] border border-plum/15 bg-white px-3 font-normal disabled:bg-blush/35" /><span className="mt-1 block font-normal text-ink/50">{t("services.customer_name_help", "Enter the name you want customers to see when browsing and booking.")}</span></label><label className="sm:col-span-2 text-sm font-bold">Description<textarea aria-label="Description" value={description} onChange={(event) => setDescription(event.target.value.slice(0, 500))} rows={3} className="mt-1 w-full rounded-[7px] border border-plum/15 p-3 font-normal" /></label><NumberField label="Duration minimum (hours)" value={durationMin} onChange={setDurationMin} step="0.25" max={24}/><NumberField label="Duration maximum (hours)" value={durationMax} onChange={setDurationMax} step="0.25" max={24}/><MoneyInput label="Base price" value={basePrice} onChange={setBasePrice} /><MoneyInput label="Maximum displayed price" value={maxPrice} onChange={setMaxPrice} /><SelectField label="Cleanup buffer" value={String(bufferMinutes)} onChange={(value) => { bufferEdited.current = true; setBufferMinutes(Number(value)); }} options={[0,15,30,45,60].map((value) => ({ value: String(value), label: `${value} minutes` }))} /></div>
        {isBraiding ? <><div className="mt-5 grid gap-4 lg:grid-cols-2"><OptionEditor title="Size Options" options={engineOptions.sizes} rows={sizes} setRows={setSizes} /><OptionEditor title="Length Options" options={engineOptions.lengths} rows={lengths} setRows={setLengths} /><ManagedAddonEditor options={availableAddons} rows={addons} setRows={setAddons} />
          <section className="rounded-[11px] border border-plum/10 bg-cream/35 p-4"><div className="flex items-center justify-between"><h3 className="font-serif text-lg text-plum">Hair / Material</h3><button type="button" onClick={addMaterial} className="flex items-center gap-1 text-sm font-bold text-magenta"><Plus size={13} />Add another</button></div><div className="mt-3 space-y-3">{materials.map((material, index) => <div key={`${material.name}-${index}`} className="rounded-[8px] border border-plum/10 bg-white p-3"><div className="grid gap-2 sm:grid-cols-2"><SelectField label="Material" value={material.name} onChange={(value) => setMaterials((current) => current.map((item, rowIndex) => rowIndex === index ? { ...item, name: value } : item))} options={engineOptions.materials.map((value) => ({ value, label: value }))} /><MoneyInput value={material.price} onChange={(value) => setMaterials((current) => current.map((item, rowIndex) => rowIndex === index ? { ...item, price: value } : item))} /><SelectField label="Longevity" value={String(material.longevity_weeks)} onChange={(value) => setMaterials((current) => current.map((item, rowIndex) => rowIndex === index ? { ...item, longevity_weeks: Number(value) } : item))} options={engineOptions.longevity.map((value) => ({ value: String(value), label: translateSource("Weeks: {value0}", { value0: formatNumber(value) }) }))} /><SelectField label="Quality" value={material.quality_grade} onChange={(value) => setMaterials((current) => current.map((item, rowIndex) => rowIndex === index ? { ...item, quality_grade: value } : item))} options={[...new Set([...engineOptions.quality,material.quality_grade].filter(Boolean))].map((value) => ({ value, label: value }))} /></div><button type="button" onClick={() => setMaterials((current) => current.filter((_, rowIndex) => rowIndex !== index))} className="mt-2 flex items-center gap-1 text-sm text-magenta"><Trash2 size={12} />Remove material</button></div>)}{!materials.length ? <Empty text="No material choices selected." /> : null}</div></section>
        </div>
        ...
        <section className="mt-4 rounded-[11px] border border-plum/10 p-4"><h3 className="font-serif text-lg text-plum">Service Image</h3><p className="mt-1 text-sm text-ink/55">Only salon-uploaded work is shown. No stock or generated fallback will appear.</p>{active?.id ? <ImageUpload bucket="style-photos" preset="service" folder={`styles/${active.id}`} label="Upload image" value={photos} multiple maxFiles={6} attachment={{ record_type: "style", record_id: String(active.id), field: "photos" }} onChange={(value) => setPhotos(Array.isArray(value) ? value : [])} onPersisted={(value) => { const next = Array.isArray(value) ? value.map(String) : []; setPhotos(next); c.setStyles((rows) => rows.map((row) => row.id === active.id ? { ...row, photos: next } : row)); }} /> : <p className="mt-3 rounded-[8px] bg-blush/30 p-3 text-xs text-plum">Save the service details first, then upload its images.</p>}</section>
        <button disabled={saving} className="mt-5 min-h-12 w-full rounded-[8px] bg-magenta text-xs font-bold text-white gc-disabled-control">{saving ? "Saving…" : "Save Service"}</button>
        {active?.id ? <button type="button" onClick={async () => { await c.removeRecord("styles", active.id!, c.setStyles); c.setSelectedStyle(null); router.push(stylesListHref); }} className="mt-3 min-h-12 w-full rounded-[8px] border border-red-200 text-xs font-bold gc-text-danger">Archive or remove service</button> : null}
...
export function StructuredStylistsEditor({ c, recordId = "" }: { c: Context; recordId?: string }) {
  const hairTeam=businessLabels(c.salon.business_type).team === "Stylists";
  const router = useRouter();
  const searchParams = useSearchParams();
  ...
  const stylistListParams = new URLSearchParams(searchParams.toString());
  ...
  return <>
    {!recordId ? <EditorTitle compactLandscape title={businessLabels(c.salon.business_type).team || "Team"} subtitle="Manage your team with consistent specialties drawn from the platform service list." action={<button type="button" onClick={startNewStylist} className="min-h-11 rounded-[8px] bg-magenta px-6 py-3 text-xs font-bold text-white"><Plus className="mr-1 inline" size={16} />{hairTeam?"Add Stylist":"Add professional"}</button>} /> : null}
    {!recordId ? <TeamWorkspace salon={c.salon} stylists={c.stylists} bookings={c.bookings} access={c.access} params={stylistListParams} services={c.styles} saveAssignments={async (professional,ids)=>{ const saved=await c.saveRecord("stylists",{assigned_service_ids:ids},String(professional.id),true); if(!saved||saved.id!==professional.id||saved.salon_id!==c.salon.id)throw new Error("The saved business record could not be verified."); c.setStylists(rows=>rows.map(row=>row.id===saved.id?saved:row)); }}/> : null}
    <MobileRecordEditor open={Boolean(recordId)} title={active ? `Edit ${active.name || (hairTeam?"stylist":"professional")}` : hairTeam?"Add stylist":"Add professional"} onClose={() => router.push(stylistListHref)}><form key={active?.id || temporaryPhotoKey} onSubmit={save}><OwnerDetailHeader hideOnMobile title={active ? `Edit ${active.name || (hairTeam?"stylist":"professional")}` : hairTeam?"Add stylist":"Add professional"} subtitle="Manage the professional profile, specialties, experience, and portfolio in one focused workspace." fallbackHref={stylistListHref} status={active ? (hairTeam?"Active stylist":"Active professional") : (hairTeam?"New stylist":"New professional")}/><EditorPanel><h2 className="font-serif text-xl text-plum">{hairTeam?"Stylist details":"Professional details"}</h2><p className="mt-1 text-sm text-ink/55">Start with any field. Nothing appears publicly until the complete form saves successfully.</p><div className="mt-4 grid gap-5 xl:grid-cols-[1fr_.75fr_1.25fr]"><div className="space-y-4"><label className="block text-sm font-bold">Name<input required value={name} onChange={(event) => setName(event.target.value.slice(0, 120))} className="mt-1 min-h-11 w-full rounded-[7px] border border-plum/15 px-3 font-normal" /></label><label className="block text-sm font-bold">Bio / Description<textarea value={bio} maxLength={250} onChange={(event) => setBio(event.target.value)} rows={6} className="mt-1 w-full rounded-[7px] border border-plum/15 p-3 font-normal" /><span className="mt-1 block text-right font-normal text-ink/45">{bio.length}/250</span></label><NumberField label="Years of Experience" value={years} onChange={setYears} step="1" max={70}/><button className="min-h-11 w-full rounded-[8px] bg-magenta text-xs font-bold text-white">{hairTeam?"Save Stylist":"Save professional"}</button>{active?.id ? <button type="button" onClick={async () => { await c.removeRecord("stylists", active.id!, c.setStylists); c.setSelectedStylist(null); router.push(stylistListHref); }} className="min-h-11 w-full rounded-[8px] border border-red-200 text-xs font-bold gc-text-danger">{hairTeam?"Archive or remove stylist":"Archive or remove professional"}</button> : null}</div><div><ImageUpload className="[&>.grid]:grid-cols-1" bucket="stylist-photos" preset="avatar" folder={active?.id ? `stylists/${active.id}` : temporaryFolder} label="Profile photo" helperText={active?.id ? "Upload or adjust the professional’s public profile photo." : "Save the professional details first, then add the profile photo."} value={avatar} attachment={active?.id ? { record_type: "stylist", record_id: String(active.id), field: "avatar_url" } : null} onChange={(value) => setAvatar(typeof value === "string" ? value : "")} onPersisted={(value) => { const next = typeof value === "string" ? value : ""; setAvatar(next); if (active?.id) c.setStylists((rows) => rows.map((row) => row.id === active.id ? { ...row, avatar_url: next || null } : row)); }} /></div><div><h3 className="font-serif text-lg text-plum">Specialties</h3><p className="mt-1 text-sm text-ink/55">Select from the centrally managed service list.</p><div className="mt-3 grid max-h-64 gap-2 overflow-y-auto rounded-[10px] border border-plum/10 p-3 sm:grid-cols-2">{masters.map((master) => <label key={master.id} className="flex items-center gap-2 text-xs"><input type="checkbox" checked={selectedNames.has(master.name)} onChange={() => setSpecialties((current) => current.includes(master.name) ? current.filter((item) => item !== master.name) : [...current, master.name])} className="accent-magenta" />{master.name}</label>)}</div><div className="mt-5"><ImageUpload className="[&>.grid]:grid-cols-1" bucket="stylist-photos" preset="gallery" multiple maxFiles={10} folder={active?.id ? `stylists/${active.id}/portfolio` : `${temporaryFolder}/portfolio`} label="Work Portfolio" helperText={active?.id ? "Select several work photos; each upload is saved independently." : "Save the professional details first, then add portfolio photos."} value={portfolio} attachment={active?.id ? { record_type: "stylist", record_id: String(active.id), field: "photos" } : null} onChange={(value) => setPortfolio(Array.isArray(value) ? value : [])} onPersisted={(value) => { const next = Array.isArray(value) ? value.map(String) : []; setPortfolio(next); if (active?.id) c.setStylists((rows) => rows.map((row) => row.id === active.id ? { ...row, photos: next } : row)); }} /></div></div></div></EditorPanel></form></MobileRecordEditor>
  </>;
}

function EditorTitle({ title, subtitle, action, compactLandscape = false }: { title: string; subtitle: string; action: React.ReactNode; compactLandscape?: boolean }) {
  return <div className={`mb-5 ${compactLandscape ? "max-lg:[@media(max-height:600px)]:mb-2 sm:max-lg:[@media(max-height:600px)]:grid sm:max-lg:[@media(max-height:600px)]:grid-cols-[auto_minmax(0,1fr)_auto] sm:max-lg:[@media(max-height:600px)]:items-center sm:max-lg:[@media(max-height:600px)]:gap-x-3" : ""}`}>
    <div className={`flex items-center justify-between gap-3 ${compactLandscape ? "sm:max-lg:[@media(max-height:600px)]:contents" : ""}`}>
      <h1 className={`min-w-0 font-serif text-[28px] font-semibold leading-tight tracking-[-.035em] text-plum sm:text-[40px] ${compactLandscape ? "sm:max-lg:[@media(max-height:600px)]:col-start-1 sm:max-lg:[@media(max-height:600px)]:row-start-1 max-lg:[@media(max-height:600px)]:text-xl" : ""}`}>{title}</h1>
      <div className={`shrink-0 ${compactLandscape ? "sm:max-lg:[@media(max-height:600px)]:col-start-3 sm:max-lg:[@media(max-height:600px)]:row-start-1" : ""}`}>{action}</div>
    </div>
    <p className={`mt-2 text-sm text-ink/65 ${compactLandscape ? "sm:max-lg:[@media(max-height:600px)]:col-start-2 sm:max-lg:[@media(max-height:600px)]:row-start-1 sm:max-lg:[@media(max-height:600px)]:mt-0" : ""}`}>{subtitle}</p>
  </div>;
}
...
function Empty({ text }: { text: string }) { return <p className="rounded-[8px] border border-dashed border-plum/15 p-4 text-center text-sm text-ink/50">{text}</p>; }
function SelectField({ label, value, onChange, options, placeholder, required = true }: { label: string; value: string; onChange: (value: string) => void; options: Array<{ value: string; label: string }>; placeholder?: string; required?: boolean }) { return <label className="text-sm font-bold">{label}<select value={value} required={required} onChange={(event) => onChange(event.target.value)} className="mt-1 min-h-11 w-full rounded-[7px] border border-plum/15 bg-white px-2 font-normal">{placeholder ? <option value="">{placeholder}</option> : null}{options.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label>; }
function NumberField({ label, value, onChange, step, max=10000 }: { label: string; value: NumericValue; onChange: (value: NumericValue) => void; step: string; max?: number }) { const decimals = step.includes(".") ? step.split(".")[1].length : 0; return <label className="text-sm font-bold">{label}<NumericInput integer={decimals === 0} decimalPlaces={decimals} min={0} max={max} value={value} onValueChange={onChange} className="mt-1 min-h-11 w-full rounded-[7px] border border-plum/15 px-3 font-normal" /></label>; }
