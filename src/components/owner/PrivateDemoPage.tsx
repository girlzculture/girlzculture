...
type Preview={sample:true;assistant:AssistantBusinessContext;business:{name:string;description:string;cover:string;photos:string[];city:string};services:RecordRow[];team:RecordRow[]};
export default function PrivateDemoPage(){
 ...
 {loading?<p role="status">{t('Loading...')}</p>:failed?<div role="alert"><p>{t('The private demo page could not load. Try again.')}</p>{reference?<p>{t('Support reference')}: <span data-no-translate>{reference}</span></p>:null}<button className="min-h-11 rounded-lg border px-4" onClick={()=>void load()}>{t('Try again')}</button></div>:data?<>
 <article className="overflow-hidden rounded-2xl border border-border"><Image unoptimized src={data.business.cover} alt="" width={1200} height={430} className="h-48 w-full object-cover sm:h-80"/><div className="space-y-3 p-5 sm:p-8"><h2 data-no-translate className="font-serif text-3xl">{data.business.name}</h2><p data-no-translate>{data.business.city}</p><p data-no-translate>{data.business.description}</p></div></article>
 <section className="my-8"><h2 className="mb-4 font-serif text-2xl">{t('Services & Pricing')}</h2><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{data.services.map(row=><article key={row.id} className="overflow-hidden rounded-xl border border-border">{row.photos?.[0]?<Image unoptimized src={row.photos[0]} alt="" width={480} height={300} className="h-44 w-full object-cover"/>:null}<div className="space-y-2 p-4"><h3 data-no-translate className="font-semibold">{row.name}</h3><p>{formatCurrency(Number(row.base_price||0))} · {row.duration_min_hours}–{row.duration_max_hours} {t('hours')}</p><p data-no-translate className="text-sm">{row.description}</p></div></article>)}</div></section>
