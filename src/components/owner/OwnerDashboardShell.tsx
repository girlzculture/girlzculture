...
export default function OwnerDashboardShell({ children, section, salonName, salonSlug, avatar, notifications = [], access = null }: { children: React.ReactNode; section: DashboardSection; salonName: string; salonSlug: string; avatar?: string | null; notifications?: DashboardNotification[]; access?: Record<string,boolean>|null }) {
  ...
  return <div className="min-h-screen bg-cream text-ink lg:grid lg:grid-cols-[220px_minmax(0,1fr)]"><RoleSessionBoundary scope="salon" />
    ...
    <nav aria-label="Owner mobile navigation" className="fixed inset-x-0 bottom-0 z-50 flex justify-around border-t border-plum/10 bg-white/95 px-1 pb-[max(7px,env(safe-area-inset-bottom))] pt-2 shadow-[0_-10px_30px_rgba(13,17,20,.08)] backdrop-blur lg:hidden">{mobileNav.map(([id,label,Icon])=>{const active=section===id||(id==="settings"&&!['overview','bookings','availability','messages'].includes(section));return <Link key={id} href={hrefFor(id)} className={`flex min-h-12 min-w-14 flex-col items-center justify-center gap-1 text-[9px] font-semibold ${active?"text-magenta":"text-ink/70"}`}><Icon size={19}/>{label}</Link>;})}</nav>
  </div>;
}
