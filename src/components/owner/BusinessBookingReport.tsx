...
export default function BusinessBookingReport({month,businessId}:{month:string;businessId:string}){
 const {translateSource:t,locale}=useI18n();const [snapshot,setSnapshot]=useState<{key:string;report:Report}|null>(null),[error,setError]=useState("");const [revision,setRevision]=useState(0),generation=useRef(0);const key=businessId+":"+month;
 const report=snapshot?.key===key?snapshot.report:null;
 useEffect(()=>{let active=true;const epoch=++generation.current;
  void Promise.resolve().then(async()=>{if(!active)return;setError("");try{const api=await createAuthenticatedApiClient("salon");if(!active)return;
  ...
  }catch(failure){if(active&&epoch===generation.current){setSnapshot(null);setError(scopedApiErrorMessage(failure,t("The appointment report could not be loaded.")));}}});
  return()=>{active=false;};
 },[key,businessId,month,revision,t]);
 ...
 return <section className={financePanel} aria-label={t("Monthly appointment report")}><h2 className="font-serif text-xl font-bold">{t("Monthly appointment report")}</h2><p className="mt-2 text-sm">{month}</p>
  ...
  {error?<div role="alert" className="mt-3"><p>{error}</p><button className={financeButton} onClick={()=>setRevision(n=>n+1)}>{t("Reload")}</button></div>:!report?<p role="status" className="mt-3">{t("Loading report…")}</p>:<>
