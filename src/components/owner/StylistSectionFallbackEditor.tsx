...
export default function StylistSectionFallbackEditor({
  ...
  const eligible = plan === "Growth" || plan === "Premium";
  ...
  return (
    <section className="mt-5 rounded-[14px] border border-plum/10 bg-white p-5">
      ...
      {!eligible ? (
        <div className="mt-4 rounded-[10px] border border-amber/30 bg-coral/10 p-4">
          <p className="text-xs font-semibold text-plum">Salon-page highlights are available on Growth and Premium plans.</p>
          <Link href="/salon/dashboard/subscription" className="mt-3 inline-flex min-h-9 items-center rounded-[7px] bg-magenta px-4 text-[10px] font-bold text-white">View upgrade options</Link>
        </div>
      ) : (
        <div className="mt-4 space-y-4">
          ...
          <button type="button" disabled={saving || (mode === "image" && !imageUrl) || (mode === "product" && !productId) || (mode === "promotion" && !promotionId)} onClick={save} className="min-h-10 rounded-[7px] bg-magenta px-5 text-[10px] font-bold text-white disabled:opacity-50">{saving ? "Saving…" : "Save salon-page replacement"}</button>
        </div>
      )}
    </section>
  );
