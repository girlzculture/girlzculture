...
export default function FinanceReportControls({from,to}:{from:string;to:string}){
  const {locale,translateSource:t}=useI18n();const [busy,setBusy]=useState(false),[error,setError]=useState('');const pending=useRef(false),active=useRef(true);
  ...
  return <div className="space-y-2"><div className="flex flex-wrap gap-2">{(['pdf','xlsx'] as const).map(format=><button key={format} type="button" disabled={busy} onClick={()=>void download(format)} className={financeButton}><Download size={16} className="mr-2 inline"/>{t(format==='pdf'?'Download PDF':'Download spreadsheet')}</button>)}</div>{error?<p role="alert" className="text-sm text-red-700">{error}</p>:null}<p className="text-xs gc-text-secondary">{t('The PDF contains summaries; the spreadsheet also includes payment and sale records for this period.')}</p></div>;
}
