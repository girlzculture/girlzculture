...
import { parseApplicationPlanQuery, SUBSCRIPTION_PLANS } from "@/lib/plans";
...
export default function SalonSignup() {
  ...
  return <form onSubmit={submit} className="space-y-5">
    ...
    <button disabled={loading} className="w-full rounded-[9px] bg-magenta py-4 font-bold text-white">{loading?"Creating account…":"Create account"}</button>
    <div className="rounded-[12px] bg-blush/45 p-4 text-sm text-plum">After you sign up, you’ll complete a short application to list your business.</div>
    <p className="text-center text-sm">Already have an account? <Link href="/business/login" className="font-semibold text-magenta">Log in</Link></p>
    <div className="grid grid-cols-3 gap-3 border-t border-plum/10 pt-5 text-center text-[10px] text-ink/70">{[[ShieldCheck,"Secure & Private"],[UsersRound,"Quality Community"],[BadgeCheck,"Built for Business"]].map(([Icon,label])=><div key={label as string}><Icon className="mx-auto mb-2 text-amber" size={23}/>{label as string}</div>)}</div>
  </form>;
}
