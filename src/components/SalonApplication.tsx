...
export default function SalonApplication() {
  ...
  return <form onSubmit={submit} className="rounded-[18px] border border-plum/10 bg-white/85 p-5 shadow-[0_20px_60px_rgba(26,18,32,.08)] sm:p-8">
    ...
    <div className="grid gap-4 sm:grid-cols-2">
      ...
      <Input label="Business license number" value={form.business_license_number} onChange={(value)=>update("business_license_number",value)} required={false} />
    ...
    <div className="mt-6 space-y-3">{["I confirm the information is accurate and I’m authorized to represent this business.","I agree to the Terms of Service and Partner Agreement.","I confirm I have permission and rights for any photos I upload."].map((label,index)=><label key={label} className="flex gap-3 text-sm"><input required type="checkbox" checked={checks[index]} onChange={(event)=>setChecks((current)=>current.map((value,itemIndex)=>itemIndex===index?event.target.checked:value))} className="accent-magenta"/>{label}</label>)}</div>
