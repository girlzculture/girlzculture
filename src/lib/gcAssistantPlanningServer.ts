...
function planningResult(tool: string, result: unknown) {
  ...
  return ["get_business_summary", "get_services_and_prices", "get_business_profile", "get_availability", "get_business_policies", "get_professionals", "get_products", "get_plan_status", "get_profile_completion", "get_earnings_summary", "get_upcoming_appointments", "get_calendar_gaps", "get_promotions"].includes(tool) ? result : null;
}
...
export async function planOwnerRequest(input: {
  ...
  const actorSchema = { ...schema, properties: { ...schema.properties, plan: { anyOf: [...choices.filter(choice => granted.has(ASSISTANT_TOOLS[choice.properties.tool.enum[0] as keyof typeof ASSISTANT_TOOLS].permission)), { type: "null" }] } } };
  const inputUnits = Buffer.byteLength(instructions + userData + JSON.stringify(actorSchema));
