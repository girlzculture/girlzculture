...
export async function planOwnerRequest(input: {
  ...
  const priorResults = (previous.data || []).filter(row => granted.has(row.permission)).map(row => ({ tool: row.tool, arguments: Object.hasOwn(ASSISTANT_TOOLS, row.tool) && ASSISTANT_TOOLS[row.tool as keyof typeof ASSISTANT_TOOLS].risk >= 3 ? null : boundedFacts(row.arguments), result: boundedFacts(planningResult(row.tool, row.result)) }));
  ...
  const userData = JSON.stringify({ request: redactSensitiveText(input.text), conversation: conversation.map(turn => ({ role: turn.role, text: redactSensitiveText(turn.text) })), previous: priorResults, ...(input.answerOnly ? {} : { platform_catalog_for_new_service_drafts: catalog.data }) });
  ...
  const actorSchema = { ...schema, properties: {
    plan: input.answerOnly ? { type: "null" } : { anyOf: [...choices.filter(choice => granted.has(ASSISTANT_TOOLS[choice.properties.tool.enum[0] as keyof typeof ASSISTANT_TOOLS].permission)), { type: "null" }] },
    reply: input.answerOnly ? { type: "string", maxLength: 900 } : { type: "null" },
    clarification: input.answerOnly ? { type: "null" } : schema.properties.clarification,
    navigate: input.answerOnly ? { type: "null" } : schema.properties.navigate,
  } };
  const phaseInstructions = instructions + (input.answerOnly ? " The authorized read for this question has now completed. Answer the actual user question using only the supplied prior results; do not propose another tool. Give a brief conversational reply, not a field-by-field dump. Only list a few examples unless specific details were requested. Arrays may be excerpts, not complete inventories. Do not claim an unavailable field is zero or absent. If evidence is missing, say which detail is unavailable. Ask at most one relevant follow-up. Never claim a write was completed." : " This is the planning step, so reply must be null. Business-data questions and follow-ups must select a fresh authorized read before any answer is written. Prior arrays are excerpts: a service missing from an excerpt is not absent or unpriced. Use get_services_and_prices with an empty query for the business inventory, or the requested service name for its details. platform_catalog_for_new_service_drafts contains platform vocabulary only, never this business's services or prices; use it only to resolve an ID when preparing a new service draft. Use clarification only to ask for a required missing detail, or for a brief greeting/capability explanation followed by an invitation to ask a question. Never put a business-data answer in clarification.");
  ...
  try {
    ...
    if (input.answerOnly ? plan.reply === null : plan.reply !== null) throw new AssistantError("ASSISTANT_INVALID_PLAN", 502);
    if (plan.plan !== null) {
