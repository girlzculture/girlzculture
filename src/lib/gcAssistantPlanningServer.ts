...
export async function planOwnerRequest(input: {
  ...
  let inputUnits = Buffer.byteLength(phaseInstructions + contextGuidance + userData + currentRequest + JSON.stringify(actorSchema));
  if (inputUnits > 64000 && !input.answerOnly) {
    ...
    inputUnits = Buffer.byteLength(phaseInstructions + contextGuidance + userData + currentRequest + JSON.stringify(actorSchema));
  }
