...
export async function createCustomerApprovedReschedule(input: {
  ...
  const emailHtml = `<div style="font-family:Arial,sans-serif;max-width:620px;margin:auto;background:#F5F7F8;padding:28px;border-radius:16px;color:#0D1114"><h1 style="font-family:Georgia,serif;color:#0D1114">New appointment times from ${escapeHtml(salon.name)}</h1><p>Your current appointment is <strong>${escapeHtml(currentTime)}</strong>. The salon proposed:</p><ul>${proposedList}</ul>${message ? `<p><strong>Salon message:</strong> ${escapeHtml(message)}</p>` : ""}<p>This proposal expires in ${expiryHours} hours. Your booking will not change unless you accept a time.</p><a href="${escapeHtml(access.url)}" style="display:inline-block;background:#0083A6;color:#fff;padding:12px 18px;border-radius:9px;text-decoration:none;font-weight:700">Review proposed times</a></div>`;
