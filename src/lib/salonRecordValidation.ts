...
export const SALON_RECORD_CONFIG: Record<string, SaveConfig> = {
  styles: {
    permission: "styles",
    label: "service",
...
export function finiteNumber(value: unknown, label: string, minimum: number, maximum: number, optional = false) {
...
export function sanitizeSalonRecord(table: string, values: Record<string, unknown>, isInsert: boolean) {
  const config = SALON_RECORD_CONFIG[table];
