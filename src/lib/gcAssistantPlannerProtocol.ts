...
const purposes: Record<AssistantTool, string> = {
  ...
  get_bookings: "Read all appointments and their authoritative IDs for a date range, including earlier appointments. Use for whole-day questions such as bookings today; preserve each recorded status.",
  ...
  get_upcoming_appointments: "Read only appointments still upcoming now within a date range. Excludes earlier appointments even when start is midnight. Use only for explicitly upcoming/remaining appointments, never a whole-day count. Describe the result as remaining/upcoming, not the full-day total.",
