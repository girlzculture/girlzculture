...
const purposes: Record<AssistantTool, string> = {
  prepare_booking_progress: "Read own booking. booking_service:{action:check_in/start/complete,reason_code,reason_detail,attested:true}; owner-stated attestation, actual out-of-window reason. booking_attendance:{action:confirm/void/reinstate,kind:no_show/late_cancellation on confirm else null,reason}. Reviewed changes_json; no cancellation/charge/notification; own incidents.",
  get_team_controls: "Owner-only users/grants, professionals, payout agreements and totals.",
  prepare_team_controls: "Read get_team_controls; owner review. permissions: team-user ID, patch grants/status Active/Inactive. arrangement: professional ID; effective_from YYYY-MM-DD today/future; kind commission:basis before_discount/after_discount,percent 0–100; booth/employee:amount_cents integer,period week/month; none:no terms. Include basis,percent,amount_cents,period; unused=null. Preserve other grants/history. No transfers/charges/invitations.",
  get_business_controls: "Owner-only settings, plan/default state, own targets. No inferred consent/delivery.",
  prepare_business_controls: "Read settings; owner-reviewed patch preserves other fields/bookings. deposits:rate,threshold_amount/rate,repeat_incident_count/rate,incident_window_days; nullable pairs,0–100%,own incidents. growth:reminder_hours(null=default),waitlist_service_ids/professional_ids(own UUIDs). rebooking:enabled,absence_days,minimum_visits,service_ids. profile:name,phone,languages; verified account workflow for email. notifications:reviews,marketing booleans. booking:slot_minutes 15/30/60,buffer_minutes 0/15/30/45/60. location:home_address_public,public_neighborhood,offers_mobile,travel_radius_miles 1–100/null,travel_fee_cents; no verified-address edits. Review exact public disclosure. Explain future opted-in reminders; never infer consent. No immediate sends/provider action.",
  ...
  prepare_service_change: "Read services. Patch catalog/price/duration/draft; size_options,length_options,addons:[{label,price_add}]; included_items:string[]; style_materials:[{name,price,longevity_weeks,quality_grade}]. Replace requested lists only; approved category options. Null ID=create:exact catalog name/ID,price,both durations. is_draft=false explicitly publishes.",
  prepare_professional_change: "Read professionals/services; patch name,bio,specialties,years_experience,is_draft,assigned_service_ids(own IDs,null=all,[]=none). Null=create; is_draft=false publishes. No login/grants; dedicated archive tool.",
  prepare_product_change: "Read products; review name,description,price,sale_price,sku,is_visible,in_person_only,product_status Draft/Active/Archived,pickup_enabled,pickup_prep_minutes,shipping_enabled,shipping_price,shipping_profile,weight_ounces,max_quantity_per_order. Null record_id=create:name/price. Explicit fulfillment choices; separate stock tools. No provider action.",
  prepare_promotion_change: "Read promotions/own targets; review title,description,public_headline,promotion_type percentage/fixed/descriptive,discount_value,discount_label,starts_at,ends_at,timezone,status Draft/Active/Paused/Archived,target_scope salon/services/products,target_ids. Null=create:title,type,value,dates,timezone. Active publishes; pause retains history. Protect deposits.",
  get_business_stock: "Read own products/supplies before changes; empty query lists. Max30/kind; complete inventory_total/matching_total. Narrow missing targets. Saved quantities/revisions.",
  prepare_stock_change: "Read stock. changes_json restock:{kind,quantity,cost_cents,note}; correction/consumption:{kind,quantity,note}; settings:{kind,track_inventory,low_stock_threshold,note}; supply_create:{name,unit,quantity,low_stock_threshold,note}; supply_archive:{note}. kind product/supply; own record_id,null=create. Explicit counts/costs. product_fulfillment:read get_products; order UUID; {fulfillment_status,carrier,tracking_number,note}; nullable prose; carrier/tracking only Shipped. No cancel/refund/payment/notification.",
  prepare_photo_change: "Read get_business_media; photo_details:{url,category,title,caption,featured,source_locale}; cover/remove:{url}. category services|before_after|space|team|client_love|other. Own saved URL; record_id=null. Removal unlinks, not storage deletion.",
  prepare_client_card_change: "Read get_client_record; own booking UUID; operation client_card; changes_json={locale,patch}. Patch requested preferences,notes,cautions,formula (instructions,color,size,length,technique,duration_minutes). No identity merges/grant edits.",
  prepare_review_reply: "Read get_reviews; own review UUID; operation review_reply; changes_json={reply}. Owner review/confirmation before publication; no invented approved wording.",
...
function withSharedDefinitions<T extends Record<string, unknown>>(schema: T): T & { $defs?: Record<string, unknown> } {
  ...
  const shared = new Map([...repeated].filter(([key, node]) => key.length * (node.count - 1) > 26 * node.count + 12)
    .map(([key, node], index) => [key, { ...node, name: index.toString(36) }]));
