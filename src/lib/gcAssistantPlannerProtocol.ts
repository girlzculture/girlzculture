...
const purposes: Record<AssistantTool, string> = {
  ...
  prepare_service_change: "Read services; edit catalog/price/duration/draft; size_options,length_options,addons:[{label,price_add}]; included_items:string[]; style_materials:[{name,price,longevity_weeks,quality_grade}]. Replace requested lists only; approved category options. Null ID=create:exact catalog name/ID,price,both durations. is_draft=false explicitly publishes.",
  prepare_professional_change: "Read professionals/services; patch name,bio,specialties,years_experience,is_draft,assigned_service_ids(own IDs,null=all,[]=none). Null=create; is_draft=false publishes. availability:null=business hours,else all Mon–Sun:{open,close,closed}; read hours first. No login/grants; dedicated archive.",
  ...
  prepare_photo_change: "get_business_media first; photo_details:{url,category,title,caption,featured,source_locale}; cover/remove:{url}. category services|before_after|space|team|client_love|other. Own saved URL; record_id=null. Removal unlinks, not storage deletion.",
  prepare_client_card_change: "get_client_record first; own booking UUID,operation=client_card,changes_json={locale,patch}. Patch requested preferences,notes,cautions,formula (instructions,color,size,length,technique,duration_minutes). No identity merges/grant edits.",
