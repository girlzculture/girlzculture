...
const purposes: Record<AssistantTool, string> = {
  ...
  get_business_controls:"Owner-only saved values, plan/default state and own targets. No inferred contact consent/delivery.",
  prepare_business_controls:"Read settings first; owner-reviewed changes_json patch; preserve other fields. deposits: rate,threshold_amount/rate,repeat_incident_count/rate,incident_window_days; nullable pairs, 0–100%, own incidents. growth: reminder_hours(null=default),waitlist_service_ids/professional_ids(own UUIDs). rebooking: enabled,absence_days,minimum_visits,service_ids. profile:name,phone,languages; email uses verified account workflow. notifications:reviews,marketing booleans. booking:slot_minutes(15/30/60),buffer_minutes(0/15/30/45/60). location:home_address_public,public_neighborhood,offers_mobile,travel_radius_miles(1–100/null),travel_fee_cents. No verified-address edits. Review exact public-address disclosure; future opted-in reminders require explanation, never infer consent. Existing bookings unchanged; no immediate sends/provider action.",
  ...
  prepare_service_change: "Read services first. Patch catalog/pricing/duration/draft fields; size_options,length_options,addons:[{label,price_add}]; included_items:string[]; style_materials:[{name,price,longevity_weeks,quality_grade}]. Lists replace only when requested. Use approved category options. Null ID=create requires exact catalog name/ID,price,both durations. is_draft=false explicitly publishes.",
  prepare_professional_change: "Read professionals/services. Patch name,bio,specialties,years_experience,is_draft,assigned_service_ids(own IDs,null=all,[]=none). Null record=create; explicit is_draft=false publishes. No login/grant changes; dedicated archive tool.",
  ...
  get_business_stock: "Search own products/supplies before changes; empty query lists. Max 30/kind, complete inventory_total/matching_total. Narrow missing targets; quantities/revisions are saved facts.",
  prepare_stock_change: "Read stock first. changes_json: restock={kind,quantity,cost_cents,note}; correction/consumption={kind,quantity,note}; settings={kind,track_inventory,low_stock_threshold,note}; supply_create={name,unit,quantity,low_stock_threshold,note}; supply_archive={note}. kind=product/supply. Own record_id, null=create. No inferred counts/costs. product_fulfillment after get_products: order UUID; {fulfillment_status,carrier,tracking_number,note}. Nullable prose; carrier/tracking only for Shipped. No cancel/refund/payment/notification.",
