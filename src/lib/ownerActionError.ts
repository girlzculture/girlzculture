...
export class OwnerActionError extends Error {
  constructor(public code: string, public reference = "",public alternatives:AppointmentAlternative[] = []) { super(code); }
}
export function ownerResponseError(body: { code?: unknown; request_id?: unknown; alternatives?:unknown }, fallback: string) {
  return new OwnerActionError(typeof body.code === "string" ? body.code : fallback,
    typeof body.request_id === "string" ? body.request_id : "",body.code==='ASSISTANT_AVAILABILITY_CONFLICT'?parseAppointmentAlternatives(body.alternatives):[]);
}
...
import {parseAppointmentAlternatives,type AppointmentAlternative} from "@/lib/assistantAppointmentAlternatives";
