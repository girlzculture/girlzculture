...
export async function readAssistantProductOperations(context: Context) {
 ...
 return { total: ordersRead.count, shown_count: orders.length, is_excerpt: orders.length !== ordersRead.count, as_of: asOf, period: "all_time", time_zone: salon.time_zone,
  ...
  definition: "Current recorded own-business product orders, newest first. Exact all-time order total is independent of the product-name search; a page cannot establish all ready/late/status totals. Items use the order's saved name/quantity, not a current price or stock count. Incomplete item reads do not prove an order has no items. Dates use this business time zone. Status/payment mode is recorded state, not verified receipt or bank settlement. No customer, address, contact, tracking, payment secret or financial amount is included. Review current orders in Products before any fulfillment action; nothing was changed or sent." };
}
