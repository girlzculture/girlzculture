...
export async function clientFailure(request: Request, error: unknown, context?: ClientContext) {
  ...
  const status = code === "CLIENT_UNAUTHENTICATED" ? 401 : code === "CLIENT_ACCESS_DENIED" ? 403 : code === "CLIENT_NOT_FOUND" ? 404 : /CHANGED|CONFLICT|LIMIT/.test(code) ? 409 : /INVALID/.test(code) || error instanceof SyntaxError ? 400 : 500;
