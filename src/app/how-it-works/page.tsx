...
export default async function HowItWorksPage() {
  ...
  return <main className="min-h-screen bg-cream pb-20 text-ink md:pb-0"><PublicHeader active="how"/>
    <section className="mx-auto w-full max-w-[1760px] px-4 pb-4 pt-3 sm:px-8 sm:pt-5 lg:px-12 2xl:px-16"><div className="grid gap-2 md:grid-cols-5">{steps.map((step,index) => { const Icon=step.icon; return <article key={step.title} className="relative rounded-[12px] border border-plum/10 bg-white/75 p-3 shadow-[0_6px_20px_rgba(13,17,20,.04)] md:min-h-[225px] md:p-5"><span className="absolute left-3 top-3 flex h-7 w-7 items-center justify-center rounded-full bg-plum text-xs font-bold text-white">{index+1}</span><Icon className="ml-10 text-amber md:mx-auto md:mt-4" size={34}/><h2 className="mt-2 font-serif text-lg font-semibold leading-tight md:mt-5 md:text-center">{step.title}</h2><p className="mt-1 text-xs leading-5 text-ink/70 md:text-center">{step.text}</p></article>; })}</div>
    ...
    </section><PublicContentSections sections={customSections}/><PublicFooter reserveMobileNavigation/><CustomerBottomNav active="home"/></main>;
}
