...
export default async function HelpPage() {
  ...
  return <main className="min-h-screen bg-cream text-ink"><PublicHeader/><section className="bg-[radial-gradient(circle_at_80%_10%,rgba(255,255,255,.18),transparent_28%),linear-gradient(130deg,#006b88,#0083a6)] px-5 py-16 text-center text-white"><p className="text-[10px] font-bold uppercase tracking-[.2em] gc-text-on-dark">{page.eyebrow || "How can we help?"}</p><h1 className="mt-3 font-serif text-5xl sm:text-6xl">{page.hero_title || page.title}</h1><p className="mx-auto mt-4 max-w-2xl text-sm leading-7 gc-text-on-dark">{page.hero_subtitle}</p></section><HelpCenter sections={faqSections} searchEnabled={searchEnabled}/><PublicContentSections sections={customSections}/><PublicFooter/></main>;
}
