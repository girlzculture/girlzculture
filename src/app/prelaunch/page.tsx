...
export const metadata: Metadata = { title: "Girlz Culture — Business onboarding", robots: { index: false, follow: false } };

const DEFAULT_DESCRIPTION =
  "Girlz Culture is onboarding beauty and wellness businesses as we prepare to launch our marketplace. Submit your application to join now.";

export default async function PrelaunchPage() {
  const [title, configuredDescription] = await Promise.all([
    getEngineText("marketplace.prelaunch_title", "A new home for your beauty business", 160),
    getEngineText("marketplace.prelaunch_description", DEFAULT_DESCRIPTION, 800),
  ]);
  const description = /\bfounding\b/i.test(configuredDescription)
    ? DEFAULT_DESCRIPTION
  ...
  return <div className="min-h-dvh bg-ivory text-ink">
    ...
    <main className="mx-auto max-w-3xl px-6 py-20 text-center">
      ...
      <Link href="/business/signup" className="mt-9 inline-flex min-h-12 items-center rounded-xl bg-magenta px-7 py-3 font-semibold text-white">Join as a business</Link>
    </main>
