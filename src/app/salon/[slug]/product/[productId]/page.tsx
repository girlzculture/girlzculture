  ...
  return (
    <main className="min-h-screen bg-cream pb-20 text-ink md:pb-0">
      ...
      <div className="mx-auto w-full max-w-[1320px] px-4 py-6 sm:px-6 lg:px-10 lg:py-12">
        ...
        <section className="mt-5 grid overflow-hidden rounded-[20px] border border-plum/10 bg-white/80 shadow-[0_18px_50px_rgba(13,17,20,0.07)] lg:grid-cols-2">
          ...
          <div className="flex flex-col justify-center p-7 sm:p-10 lg:p-14">
            ...
            {offer ? <div className="mt-4"><span className="inline-flex rounded-full bg-amber/20 px-3 py-1 text-[10px] font-bold gc-text-warning">{promotionLabel(offer.promotion)}</span><p className="mt-2 text-sm gc-text-secondary line-through">${catalogPrice.toFixed(2)}</p><p className="text-2xl font-bold text-magenta">${offer.price.total.toFixed(2)}</p><p className="mt-2 text-[11px] text-plum">{offer.promotion.public_headline || offer.promotion.title}</p></div> : product.sale_price !== null && product.sale_price !== undefined ? <div className="mt-4 flex items-end gap-3"><p className="text-sm gc-text-secondary line-through">${Number(product.price || 0).toFixed(2)}</p><p className="text-2xl font-bold text-magenta">${catalogPrice.toFixed(2)}</p></div> : <p className="mt-4 text-2xl font-bold text-ink">${catalogPrice.toFixed(2)}</p>}
