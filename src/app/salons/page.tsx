...
export default async function SalonsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const [{ data: salons }, { data: styles }, { data: subscriptions }] = await Promise.all([
    supabase.from("salons").select("*"),
    supabase.from("styles").select("salon_id,name,price_display_min"),
    supabase.from("subscriptions").select("salon_id,subscription_tier"),
  ]);
  const mapped: DiscoverySalon[] = (salons || []).map((salon, index) => {
    ...
    const tier = (subscriptions || []).find((item) => item.salon_id === salon.id)?.subscription_tier || salon.subscription_tier || "Basic";
    return { id: salon.id, name: salon.name || "Girlz Culture Salon", slug: salon.slug || salon.id, neighborhood: salon.neighborhood || "Nearby", city: salon.address_city || "New York", rating: Number(salon.rating_overall || 4.8), reviewCount: Number(salon.review_count || 0), image: salon.cover_photo_url || fallbackImages[index % fallbackImages.length], startingPrice: prices.length ? Math.min(...prices) : 120 + index * 10, tier, styles: salonStyles.map((item) => item.name).filter(Boolean), nextAvailability: index % 2 ? "Today 4:00 PM" : "Tomorrow 10:00 AM", latitude: salon.latitude, longitude: salon.longitude };
  });
