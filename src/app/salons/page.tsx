...
export default async function SalonsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <main className="min-h-screen bg-cream pb-20 text-ink md:pb-0"><PublicHeader active="salons" /><section className="mx-auto w-full max-w-[1760px] px-4 pb-5 pt-3 sm:px-8 sm:pt-6 lg:px-12 2xl:px-16"><h1 className="font-serif text-[32px] font-semibold leading-none tracking-[-0.04em] text-ink sm:text-[56px]">Find salons that fit your style<span className="text-magenta">.</span></h1><p className="mt-1 text-xs sm:mt-2 sm:text-base">Search by service, price, distance, rating, and real availability.</p><div className="mt-3 sm:mt-4"><SalonDiscovery initialSalons={mapped} initialStyle={typeof query.style === "string" ? query.style : ""} initialLocation={typeof query.location === "string" ? query.location : ""} /></div></section><TrustStrip /><CustomerBottomNav active="search" /></main>;
}
