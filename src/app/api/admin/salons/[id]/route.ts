...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const [{ data: salon, error }, subscription, audit, application, future] = await Promise.all([
    ...
    return Response.json({ salon, subscription: subscription.data, status_history: audit.data || [], application: application.data, future_booking_count: future.count || 0 }, { headers: { "Cache-Control": "private, no-store" } });
  } catch (error) {
