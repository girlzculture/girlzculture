...
export async function POST(request: Request) {
  try {
    ...
    const email = cleanEmail(body.email); const name = cleanText(body.name, 120); const role = ["Manager","Front Desk","Stylist","Staff"].includes(cleanText(body.role, 30)) ? cleanText(body.role, 30) : "Staff"; const stylistId = cleanText(body.stylist_id, 50) || null;
    ...
    const { data, error } = await admin.from("salon_team_members").upsert({ salon_id: salon.id, user_id: invited.user.id, stylist_id: stylistId, email, name, role, permissions: permissions(body.permissions), status: invited.user.last_sign_in_at ? "Active" : "Invited", invited_by: user.id, activated_at: invited.user.last_sign_in_at || null }).select().single(); if (error) throw error;
