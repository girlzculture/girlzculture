...
async function POSTHandler(request: Request) {
  try {
    ...
    if (identity?.status === "Disabled") return Response.json({ path: "/login", role: "disabled", error: "You do not have permission to access this account." }, { status: 403 });
    ...
    noteOperationalFailure("Role destination lookup failed", error);
    return Response.json({ path: "/login", role: "unknown", error: "Unable to verify account role" }, { status: 500 });
  }
}
export const POST = withOperationalMonitoring(routeMonitoringProfile("/api/auth/destination", "POST"), POSTHandler);
