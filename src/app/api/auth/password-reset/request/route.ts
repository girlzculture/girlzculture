...
export async function POST(request: Request) {
  try {
    ...
    if (channel === "sms") {
      ...
      const delivery = await sendEmail(email, "Your Girlz Culture password reset code", `<h1>Password reset</h1><p>Your one-time code is:</p><p style="font-size:28px;font-weight:700;letter-spacing:4px">${code}</p><p>This code expires in 10 minutes. If you did not request it, you can ignore this email.</p>`) as { skipped?: boolean };
