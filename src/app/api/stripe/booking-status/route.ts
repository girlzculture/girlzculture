...
async function GETHandler(request: Request) {
  ...
  const {data:booking}=intent.booking_id?await admin.from("bookings").select("id,public_reference,confirmation_code,status,appointment_datetime,business_policy_revision_id,business_policy_version,business_policy_snapshot,deposit_rule_snapshot,estimated_total,deposit_amount,balance_due").eq("id",intent.booking_id).single():{data:null};
