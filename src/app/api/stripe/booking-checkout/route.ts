...
export async function POST(request: Request) {
  try {
    ...
    const { data: salon } = await admin.from("salons").select("id,slug,name,status,subscription_status,capacity").eq("id",salonId).single();
    ...
    const { data: style } = await admin.from("styles").select("*").eq("id",styleId).eq("salon_id",salonId).single();
    ...
    const guestName=cleanText(body.guest_name,120), guestEmail=cleanEmail(body.guest_email), guestPhone=cleanText(body.guest_phone,30);
