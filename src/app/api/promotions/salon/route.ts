...
async function GETHandler(request: Request) {
  ...
  try {
    ...
    if (!salon.data || salon.data.status !== "Active" || salon.data.is_discoverable !== true || !hasPlanFeature(salon.data.subscription_tier, "promotions")) return Response.json({ error: "This salon offer is not available." }, { status: 404 });
