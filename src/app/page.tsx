...
import { getContentPage, type ContentCard, type ContentSection } from "@/lib/content";
...
async function loadHomepageSections() {
  try {
    ...
    await capturePublicPageFailure(error, "homepage", "load-section-controls");
...
export default async function Home({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const homepageSections = DEFAULT_HOME_SECTIONS.map((section) => {
    ...
    return {
      ...merged,
      description: ["salons_near_you", "featured_salons", "trending_picks"].includes(section.section_key) ? null : merged.description,
    };
  }).filter((section) => section.is_visible).sort((left, right) => left.sort_order - right.sort_order);
  const sourcePromoSection = findHomeHeroSection(homeContent.sections || []);
  const promoSection = canonicalHomeHeroSection(sourcePromoSection);
  const promoLayout = heroPresentationLayout(sourcePromoSection);
  ...
  const [promotionCards, cardCounts] = await Promise.all([
    resolvePublishedHomepagePromotions(configuredPromotionCards, undefined, promoSection.display_limit || 8),
    Promise.all([
  ...
  const contentSections = homeContent.sections?.filter((section) => section !== sourcePromoSection) || [];
      ...
      <div className="gc-home-content mx-auto w-full max-w-[1760px] px-4 pt-2 sm:px-6 sm:pt-3 lg:px-10 xl:px-12 2xl:px-16">
        {homepageSections.map((section, index) => (
          <Fragment key={section.section_key}>
            ...
            <HomepageRow section={section} promotionCards={promotionCards} promotionSection={promoSection} promotionLayout={promoLayout} nearbyCardCount={nearbyCardCount} featuredCardCount={featuredCardCount} productCardCount={productCardCount} trendingCardCount={trendingCardCount}/>
          </Fragment>
        ))}
        ...
        <section id="how-it-works" className="mb-3 rounded-[16px] bg-blush/70 px-4 py-4 sm:px-7 lg:grid lg:grid-cols-[200px_1fr] lg:items-center lg:gap-7">
          ...
          <div className="mt-4 grid grid-cols-3 gap-3 lg:mt-0">
            {[
              ...
              return <div key={step.title} className="relative flex flex-col items-center text-center sm:flex-row sm:text-left"><span className="inline-flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-white text-magenta shadow-sm"><Icon size={23} strokeWidth={1.9} /></span><span className="mt-2 sm:ml-3 sm:mt-0"><span className="block font-serif text-[16px] font-semibold text-ink">{step.title}</span><span className="mt-1 hidden max-w-[145px] text-[10px] leading-4 text-ink/60 sm:block">{step.description}</span></span>{index < 2 ? <ArrowRight aria-hidden="true" size={16} className="absolute -right-2 top-4 hidden text-plum/35 lg:block" /> : null}</div>;
            })}
          </div>
        </section>
      </div>
...
function HomepageRow({ section,promotionCards,promotionSection,promotionLayout,nearbyCardCount,featuredCardCount,productCardCount,trendingCardCount }: { section: HomeSection;promotionCards:ContentCard[];promotionSection:ManagedContentSection;promotionLayout:HeroPresentationLayout;nearbyCardCount:number;featuredCardCount:number;productCardCount:number;trendingCardCount:number }) {
...
function HomepageSearch() {
  return <section data-home-search aria-label="Search Girlz Culture" className="gc-desktop-home-search pb-3 pt-1 sm:pb-4 sm:pt-2"><SearchComposer /></section>;
}
