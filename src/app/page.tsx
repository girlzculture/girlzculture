...
const DEFAULT_HOME_SECTIONS: HomeSection[] = [
  ...
  },
  {
    ...
    description: "Reserve salon favorites for local pickup.",
    is_visible: true,
    sort_order: 5,
  },
  {
    section_key: "trending_now",
    title: "Trending Now",
    description: null,
    is_visible: false,
    sort_order: 6,
...
const DEFAULT_PROMOTION_CARDS: ContentCard[] = [
  {
    ...
    body: "See verified braiding salons serving Harlem and the Bronx.",
    media_url: "/images/salon-warm.jpg",
    href: "/salons",
    cta_label: "Find a salon",
    alt_text: "Warm, modern braiding salon interior",
    status: "Active",
  },
  {
    id: "pilot-knotless",
    content_type: "image",
    title: "Knotless braids, clear prices",
    body: "Compare real service details before you reserve.",
    media_url: "/images/braids-knotless.jpg",
    href: "/styles?style=knotless-braids",
    cta_label: "Browse knotless",
    alt_text: "Client wearing knotless braids",
    status: "Active",
  },
  {
    id: "pilot-box",
    content_type: "image",
    title: "Explore box braids",
    body: "Choose a salon, stylist, length, and available time.",
    media_url: "/images/braids-box.jpg",
    href: "/styles?style=box-braids",
    cta_label: "Explore styles",
    alt_text: "Detailed box braid hairstyle",
    status: "Active",
  },
  {
    id: "pilot-cornrows",
    content_type: "image",
    title: "Cornrow specialists",
    body: "Discover local professionals and verified client reviews.",
    media_url: "/images/braids-cornrows.jpg",
    href: "/styles?style=cornrows",
    cta_label: "See specialists",
    alt_text: "Client wearing neat cornrows",
    status: "Active",
  },
  {
    id: "pilot-book",
    ...
    body: "Secure an appointment with a clear reservation deposit.",
  ...
  },
  {
    id: "pilot-how",
    content_type: "image",
    title: "How Girlz Culture works",
    body: "From discovery to a verified review, see every step.",
    media_url: "/images/salon-modern.jpg",
    href: "/how-it-works",
    cta_label: "How it works",
    alt_text: "Bright contemporary beauty salon",
    status: "Active",
  },
  {
    id: "pilot-partner",
    ...
    body: "Manage services, availability, bookings, and your public page.",
    media_url: "/images/salon-blush.jpg",
    href: "/partner",
    cta_label: "Partner with us",
    alt_text: "Blush-toned salon interior",
    status: "Active",
  },
  {
    id: "pilot-trust",
    ...
    body: "Book from transparent salon profiles with verified feedback.",
    media_url: "/images/salon-dark.jpg",
    href: "/safety",
    cta_label: "Safety and trust",
    alt_text: "Premium dark-toned salon interior",
    status: "Active",
  },
];
...
export default async function Home({
  ...
  const homeContent = await getContentPage("home", {
    slug: "home",
    title: "Home",
    hero_title: "Book with Confidence.",
    hero_subtitle: "",
    hero_image_url: "/images/braids-knotless.jpg",
    sections: [],
  });
  ...
  const sectionOverrides = new Map(
    ((sectionData || []) as HomeSection[]).map((section) => [
      section.section_key,
      section,
    ]),
  );
  ...
  const promoSection = homeContent.sections?.find(
    (section) =>
      section.type === "promo_rail" && section.is_visible !== false,
  );
  const configuredPromotionCards = promoSection?.cards?.length
    ? promoSection.cards
    : DEFAULT_PROMOTION_CARDS;
  ...
  const contentSections =
    homeContent.sections?.filter((section) => section.type !== "promo_rail") ||
    [];
  const [
    nearbyCardCount,
    featuredCardCount,
  ...
  return (
    <main
      data-homepage-variant={depthPreview ? "depth" : "standard"}
      className={`min-h-screen overflow-x-clip bg-cream pb-20 text-ink md:pb-0 ${
        depthPreview ? "gc-home-depth" : ""
      }`}
    >
      <PublicHeader />
      ...
      <div className="gc-home-content mx-auto w-full max-w-[1760px] px-4 pt-2 sm:px-6 sm:pt-3 lg:px-10 xl:px-12 2xl:px-16">
        {homepageSections.map((section, index) => (
          <Fragment key={section.section_key}>
            ...
            <HomepageRow
              ...
              nearbyCardCount={nearbyCardCount}
              featuredCardCount={featuredCardCount}
              productCardCount={productCardCount}
              trendingCardCount={trendingCardCount}
            />
          </Fragment>
        ))}
        ...
        <section
          id="how-it-works"
          className="mb-3 rounded-[16px] bg-blush/70 px-4 py-4 sm:px-7 lg:grid lg:grid-cols-[180px_1fr] lg:items-center lg:gap-7"
        >
          <h2 className="font-serif text-[22px] font-semibold tracking-[-0.03em] text-ink">
            How it works
          </h2>
            ...
            {[
              {
                number: "1",
                title: "Find",
                description: "Search styles or salons near you.",
              },
              {
                number: "2",
                title: "Book",
                description: "Choose an available time and review the price.",
              },
              {
                number: "3",
                title: "Go",
                description: "Attend your appointment and leave a review.",
              },
            ].map((step) => (
              <div
                key={step.title}
                className="rounded-[10px] bg-white px-3 py-3 text-center sm:text-left"
              >
                <span className="text-[11px] font-bold text-magenta">
                  Step {step.number}
                </span>
                <span className="mt-1 block font-serif text-[16px] font-semibold text-ink">
                  {step.title}
                </span>
                <span className="mt-1 hidden text-[11px] leading-5 text-ink/65 sm:block">
                  {step.description}
                </span>
              </div>
            ))}
...
function HomepageRow({
  ...
  section: HomeSection;
  promotionCards: ContentCard[];
  nearbyCardCount: number;
  featuredCardCount: number;
  productCardCount: number;
  trendingCardCount: number;
}) {
  ...
  if (section.section_key === "salons_near_you")
    return (
      <NearbySalonPlacement
        title={section.title}
        description={section.description}
        maxCards={nearbyCardCount}
      />
    );
  if (section.section_key === "featured_salons")
    return (
      <FeaturedSalonPlacement
        title={section.title}
        description={section.description}
        maxCards={featuredCardCount}
      />
    );
  if (section.section_key === "featured_products")
    return (
      <FeaturedProductPlacement
        title={section.title}
        description={section.description}
        maxCards={productCardCount}
      />
    );
  if (
    section.section_key === "trending_picks" ||
    section.section_key === "trending_now"
  )
    return (
      <TrendingVideoPlacement
        title={section.title}
        description={section.description}
        maxCards={trendingCardCount}
      />
    );
  return null;
}
...
function HomepageSearch() {
  return (
    <section
      data-home-search
      aria-label="Search Girlz Culture"
      className="gc-desktop-home-search pb-3 pt-1 sm:pb-4 sm:pt-2"
    >
      <SearchComposer />
    </section>
  );
