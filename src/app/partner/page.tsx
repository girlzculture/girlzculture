...
export default async function PartnerPage() {
  ...
  return <main className="min-h-screen overflow-x-hidden bg-cream text-ink">
    ...
    <section className="mx-auto grid w-full max-w-[1760px] items-center gap-8 overflow-hidden px-5 py-8 md:grid-cols-[minmax(0,1fr)_minmax(0,.9fr)] md:py-10 lg:px-16">
      ...
      <div className="relative min-h-[340px] min-w-0 overflow-hidden rounded-[22px] bg-blush sm:min-h-[460px] sm:rounded-[28px]"><div className="absolute inset-0 bg-[url('/images/hero-braids.jpg')] bg-cover bg-[center_30%]"/>{photoLabels.length ? <div className="absolute inset-x-4 bottom-4 grid gap-2 rounded-[16px] bg-white/90 p-4 backdrop-blur sm:inset-x-6 sm:grid-cols-3">{photoLabels.map(label => <p key={label} className="text-center text-xs font-semibold leading-5 text-plum">{label}</p>)}</div> : null}</div>
    </section>
