...
export default defineConfig({
  ...
  webServer: servers.map((server, index) => index === 0 ? {
    ...
    env: { ...server.env, PLAYWRIGHT_ACCEPTANCE_SUPABASE_URL: provider, P0_PRELAUNCH_FIXTURE: 'true' },
  } : {
    ...server, command: 'npm run dev -- -H 127.0.0.1 -p 3108', url, reuseExistingServer: false,
    env: { ...server.env, CUSTOMER_MARKETPLACE_LIVE: '', NEXT_PUBLIC_SITE_URL: url, NEXT_PUBLIC_SUPABASE_URL: provider },
  }),
