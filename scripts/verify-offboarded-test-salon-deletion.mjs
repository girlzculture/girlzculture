...
const migration = read("supabase/migrations/20260804210000_offboarded_test_salon_protected_deletion.sql");
...
const authoritativePublication = read("supabase/migrations/20260727230000_publication_activation_and_stylist_slugs.sql");

// The destructive path must be impossible for an active salon, an unmarked
// real salon, a non-super-admin, or a request without recent high-risk auth.
assert.match(migration, /Only a Super Admin can permanently delete an offboarded test salon/);
assert.match(migration, /lower\(trim\(coalesce\(v_salon\.status, ''\)\)\) <> 'offboarded'/);
assert.match(migration, /registry\.record_type = 'salon'/);
assert.match(migration, /registry\.record_id = p_salon_id::text/);
assert.match(migration, /This salon is not explicitly registered as test data/);
assert.match(migration, /maintenance\.test_data_enabled/);
assert.match(migration, /status = 'Published'/);
assert.match(migration, /if not coalesce\(v_enabled, false\)/);
assert.match(migration, /p_environment not in \('development', 'preview', 'production'\)/);
...
assert.match(migration, /length\(trim\(coalesce\(p_reason, ''\)\)\) < 8/);
assert.match(api, /assertRecentHighRiskVerification\(admin,user\.id,"admin"\)/);
assert.match(api, /adminUser as\{is_super_admin\?:boolean\}/);
...
assert.doesNotMatch(api, /NODE_ENV==="production"\)return "production"/);
...
assert.doesNotMatch(api, /actions:base\.actions\.filter\(item=>item!=="delete_test"\)/);
assert.match(migration, /grant execute on function public\.admin_delete_offboarded_test_salon\(uuid,uuid,text,text,text,jsonb\)\s+to service_role/);

// Permanent deletion means operational removal backed by a hidden tombstone;
// the migration must never cascade-delete financial or booking evidence.
...
const tombstoneUpdate = migration.match(/update public\.salons\s+set name = 'Deleted test salon '[\s\S]*?where id = p_salon_id;/)?.[0];
...
for (const clearedField of [
  ...
  assert.match(tombstoneUpdate, new RegExp(`\\b${clearedField}\\s*=\\s*null\\b`));
}
...
assert.match(migration, /retained_salon\.stripe_account_id is distinct from v_salon\.stripe_account_id/);
assert.match(migration, /salon_vanity_slug_available\(v_salon\.vanity_slug, null\)/);
assert.match(migration, /salon_slug_redirects[\s\S]*retired_at = coalesce\(retired_at, now\(\)\)/);
...
assert.match(migration, /create or replace function public\.prepare_salon_geocoding\(\)[\s\S]*if new\.deleted_at is not null then[\s\S]*new\.address_fingerprint := null/);
assert.match(migration, /update of address_street,[\s\S]*deleted_at[\s\S]*execute function public\.prepare_salon_geocoding/);
assert.match(migration, /create or replace function public\.reconcile_salon_publication\([\s\S]*if v_before\.deleted_at is not null then[\s\S]*set slug = null,[\s\S]*vanity_slug = null/);
...
assert.match(migration, /final_salon\.address_needs_review is distinct from true/);
assert.match(migration, /retained or regenerated a public identifier after lifecycle and address triggers/);

// This migration redefines publication reconciliation only to add the deleted
// early stop. It must retain every authoritative activation/override branch
// introduced by the latest prior definition.
const authoritativeReconcile = authoritativePublication.match(/create or replace function public\.reconcile_salon_publication\([\s\S]*?\n\$\$;/)?.[0];
const hardenedReconcile = migration.match(/create or replace function public\.reconcile_salon_publication\([\s\S]*?\n\$\$;/)?.[0];
assert.ok(authoritativeReconcile, "Expected the authoritative publication reconciliation function.");
assert.ok(hardenedReconcile, "Expected the deletion-hardened publication reconciliation function.");
for (const marker of [
  ...
  assert.ok(authoritativeReconcile.includes(marker), `Authoritative reconciliation is missing ${marker}`);
  assert.ok(hardenedReconcile.includes(marker), `Hardened reconciliation regressed ${marker}`);
}
assert.match(hardenedReconcile, /if v_before\.deleted_at is not null then/);

// UI must distinguish ordinary offboarding from the protected test-only action,
// show exact dependency effects, and require the destructive acknowledgement.
assert.match(manager, /Permanently delete test salon/);
assert.match(manager, /Offboard/);
assert.match(manager, /Operational records changed/);
assert.match(manager, /History explicitly retained/);
assert.match(manager, /hidden anonymized tombstone/);
assert.match(manager, /acknowledgeRetention/);
assert.match(manager, /confirmation_phrase/);
assert.match(manager, /Permanent deletion unavailable/);
assert.match(manager, /protectedDeletionPreviewUnavailable/);
assert.match(manager, /setActions\(\["offboard","delete_test"\]\)/);
...
// Tombstones must disappear from both salon inventory and aggregate admin data.
...
console.log("Verified protected deletion for explicitly marked, offboarded test salons with immutable financial history and audit evidence.");
