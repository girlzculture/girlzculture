...
try{
 ...
 for(const file of ['verify-business-service-contribution.sql','verify-business-deposits.sql','verify-business-inventory.sql','verify-business-reviews.sql','verify-business-client-cards.sql','verify-google-business-profile.sql','verify-master-booking-reports.sql','verify-master-rebooking-reminders.sql']){
  sql(readFileSync('scripts/sql/'+file,'utf8'));console.log(file+': PASS');
 }
 console.log(`Master Build representative upgrade: 188 + ${pending.length} migrations; 10 preservation checks and 8 SQL suites passed.`);
