...
assert.match(complimentaryMigration,/complimentary_admin/);
assert.match(complimentaryMigration,/complimentary_reason/);
assert.match(complimentaryMigration,/complimentary_approved_by/);
assert.match(complimentaryMigration,/complimentary_approved_at/);
assert.match(adminApi,/placementBasis === "complimentary_admin"/);
assert.match(adminApi,/featured_campaign_audit/);
assert.match(placement,/Featured/);
assert.doesNotMatch(placement,/Sponsored/);
assert.match(placement,/maxCards=12/);
assert.match(placement,/limit = viewAll \? 24 : Math\.max\(1,Math\.min\(24,Math\.round\(maxCards\)\)\)/);
assert.match(home,/homepage\.featured_card_count/);
assert.match(placement,/We will not fill this row with distant\s+or ineligible salons/);
...
assert.match(admin, /complimentary placement/i);
