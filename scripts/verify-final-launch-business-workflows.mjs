...
for (const pattern of [/customer_q/,/bookingAvailability/,/reserve_booking_checkout/,/Send Stripe deposit link|send_link/,/waive/,/paid_outside/,/collect_at_salon/,/record_management_events/,/stripeRequest<[^>]+>\(\s*"\/checkout\/sessions"/s]) assert.match(manualApi, pattern);
...
for (const pattern of [/stripeGet<StripeAccount>/,/admin_reserve_booking_payout/,/stripeRequest<StripeTransfer>\(\s*"\/transfers"/s,/source_transaction/,/idempotencyKey/,/admin_finalize_booking_payout/,/Do not create a new payout/]) assert.match(payoutApi, pattern);
