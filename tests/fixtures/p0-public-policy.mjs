...
export async function p0PublicPolicyFixture(request, response, url, json, readJson) {
  if (url.pathname.startsWith('/__fixtures/p0-public-policy/') && request.method === 'POST') {
    ...
    try {
      ...
      else if ([1, 2].includes(input.version)) records.set(id, {version:input.version,priced:input.priced===true});
  ...
  const {version,priced} = records.get(businessId);
  ...
  const rule={id:businessId,salon_id:businessId,rate:10,threshold_amount:null,threshold_rate:null,repeat_incident_count:null,repeat_incident_rate:null,incident_window_days:365};
