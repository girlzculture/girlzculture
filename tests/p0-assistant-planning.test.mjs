...
test('shared planner definitions preserve the complete pre-factoring owner schema and every permitted tool argument', () => {
  ...
  const legacy=structuredClone(expanded);legacy.properties.decision.anyOf=legacy.properties.decision.anyOf.filter(row=>!['prepare_service_change','prepare_professional_change','prepare_product_change','prepare_promotion_change','get_business_controls','prepare_business_controls','prepare_professional_archive','get_finance_records','prepare_finance_record','get_business_stock','prepare_stock_change','prepare_photo_change','prepare_client_card_change','prepare_review_reply'].includes(row.properties.tool?.enum[0]));
