...
function fixture({tool='prepare_product_change',salon=id,error=null,moderation='allow',values={name:'Own product',description:'Reviewed',price:25}}={}){
 const calls=[];const ctx={salon:{id},user:{id:'actor'},admin:{async rpc(name,input){calls.push(name);assert.equal(input.p_salon,id);assert.equal(input.p_actor,'actor');return {error,data:{salon_id:salon,before:{record:{price:20},fingerprint:'old'},payload:{tool:'prepare_product_change',values,changes:{price:25}}}};}}};
 ...
 return {calls,run:()=>helper.prepareAssistantCatalog(ctx,tool,args(tool==='prepare_promotion_change'?{public_headline:'Headline'}:tool==='prepare_professional_change'?{specialties:['Specialty']}:{price:25}))};
}
