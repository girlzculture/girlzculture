...
test("workflow preserves normal browsers and gates every production command after local migration checks", () => {
  ...
  assert.ok(verify.steps.some(step => step.uses === "actions/download-artifact@v5" && step.with.pattern === "browser-shard-*-results"));
  ...
  assert.ok(releaseGate.steps.some(step => step.uses === "actions/download-artifact@v5" && step.with["merge-multiple"] === true));
