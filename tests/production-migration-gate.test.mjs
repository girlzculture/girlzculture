...
test("workflow preserves normal browsers and gates every production command after local migration checks", () => {
  ...
  const { assistant_focused: assistant, browser_shards: browser, verify, "verify-migrations": migrations, migrate } = workflow.jobs;
  ...
  assert.deepEqual(verify.needs, ["assistant_focused", "browser_shards"]);
  assert.match(verify.if, /needs\.browser_shards\.result == 'success'/);
  ...
  assert.equal(browserStep.run, "node scripts/verify-browser-shard-results.mjs");
  assert.equal(browser.needs, "assistant_focused");
  assert.equal(browser.if, "github.event_name != 'workflow_dispatch'");
  assert.equal(browser["timeout-minutes"], 60);
  assert.equal(browser.strategy["fail-fast"], false);
  assert.deepEqual(browser.strategy.matrix.shard, [1, 2, 3, 4, 5, 6, 7, 8]);
  assert.equal(browser.env.PLAYWRIGHT_BROWSER_SHARDS, "8");
  assert.equal(browser.env.PLAYWRIGHT_SHARD_INDEX, "${{ matrix.shard }}");
  assert.ok(browser.steps.some(step => step.run === "node scripts/run-browser-shards.mjs"));
  assert.ok(browser.steps.some(step => step.run === "npx playwright install --with-deps chromium firefox webkit"));
  assert.ok(browser.steps.every(step => !step["continue-on-error"]));
  const browserArtifacts = verify.steps.find(step => step.uses === "actions/download-artifact@v5");
  assert.equal(browserArtifacts.with.pattern, "browser-shard-*-${{ github.run_id }}-*");
  assert.equal(browserArtifacts.with.path, "browser-artifacts");
  assert.equal(browserArtifacts.with["merge-multiple"], false);
  assert.ok(verify.steps.some(step => step.run === "node scripts/collect-browser-shard-results.mjs browser-shard"));
  const browserUpload = browser.steps.find(step => step.name === "Preserve shard results even on failure");
  assert.equal(browserUpload.with.name, "browser-shard-${{ matrix.shard }}-${{ github.run_id }}-${{ github.run_attempt }}");
  assert.ok(!browserUpload.with.overwrite, "Original attempts must remain available");
  const releaseWorkflow = yaml.load(readFileSync(new URL("../.github/workflows/final-launch-release-candidate-validation.yml", import.meta.url), "utf8"));
  const releaseBrowser = releaseWorkflow.jobs["browser-acceptance"];
  assert.deepEqual(releaseBrowser.strategy.matrix.shard, browser.strategy.matrix.shard);
  assert.equal(releaseBrowser.strategy["fail-fast"], false);
  assert.equal(releaseBrowser["timeout-minutes"], 60);
  assert.equal(releaseBrowser.env.PLAYWRIGHT_BROWSER_SHARDS, "8");
  assert.equal(releaseBrowser.env.PLAYWRIGHT_SHARD_INDEX, "${{ matrix.shard }}");
  assert.ok(releaseBrowser.steps.some(step => step.run === "node scripts/run-browser-shards.mjs"));
  assert.ok(releaseBrowser.steps.every(step => !step["continue-on-error"]));
  const releaseGate = releaseWorkflow.jobs["release-candidate"];
  assert.ok(releaseGate.needs.includes("browser-acceptance"));
  assert.ok(releaseGate.steps.some(step => step.run === "node scripts/verify-browser-shard-results.mjs"));
  const releaseArtifacts = releaseGate.steps.find(step => step.uses === "actions/download-artifact@v5");
  assert.equal(releaseArtifacts.with.pattern, "p0-browser-shard-*-${{ github.run_id }}-*");
  assert.equal(releaseArtifacts.with.path, "browser-artifacts");
  assert.equal(releaseArtifacts.with["merge-multiple"], false);
  assert.ok(releaseGate.steps.some(step => step.run === "node scripts/collect-browser-shard-results.mjs p0-browser-shard"));
  const releaseUpload = releaseBrowser.steps.find(step => step.uses === "actions/upload-artifact@v5");
  assert.equal(releaseUpload.with.name, "p0-browser-shard-${{ matrix.shard }}-${{ github.run_id }}-${{ github.run_attempt }}");
  assert.ok(!releaseUpload.with.overwrite, "Original failure artifacts must remain available");
  ...
  assert.match(shardRunner, /"node_modules\/next\/dist\/bin\/next", "build", "--webpack"/);
  assert.match(shardRunner, /path\.join\(outputRoot, `shard-\$\{shard\}\.log`\)/);
  assert.match(shardRunner, /const workers = 1;/);
  assert.match(shardRunner, /const shard = Number\(process\.env\.PLAYWRIGHT_SHARD_INDEX\)/);
  assert.doesNotMatch(shardRunner, /shardConcurrency|runNextShard/);
  assert.match(shardRunner, /"--reporter=list,json"/);
