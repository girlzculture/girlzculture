...
for(const [width,height] of [[390,844],[768,1024],[1440,1000],[844,390]]){
 test(`Master demo bookings and sample subscription remain populated at ${width}x${height}`,async({page},info)=>{
  ...
  Object.assign(f.business,{is_demo:true,name:'Culture House',gallery_photos:['https://girlzculture.com/images/salon-warm.jpg']});
  const base={...f.records.bookings[0],payment_mode:'test'};
  f.records.bookings=[{...base,is_demo:true,guest_name:'Emma Bennett',status:'Completed',balance_due:168,recorded_balance_cents:0},
  ...
  await expect(bookings.getByText('Emma Bennett',{exact:true}).filter({visible:true})).toBeVisible();
  await expect(bookings.getByText('Sandbox excluded',{exact:true})).toHaveCount(0);
  const paid=bookings.getByText('Emma Bennett',{exact:true}).filter({visible:true});
  const row=width<1280?paid.locator('xpath=ancestor::button[1]'):paid.locator('xpath=ancestor::tr[1]');
  await expect(row).toContainText('$0.00');await expect(row).toContainText('Paid');
  await page.reload();await expect(bookings.getByText('Emma Bennett',{exact:true}).filter({visible:true})).toBeVisible();
  ...
  await expect(rows.last()).toContainText('CH-SAMPLE-0');
  for(const row of await rows.all())await expect(row).toContainText('$199.00');
  await expect(history).not.toContainText(/signed Stripe webhook|Fictional|Private demonstration|Simulated|Test mode/);
  await expect(page.getByText('Private demonstration — sample data',{exact:true})).toHaveCount(0);
  ...
  await expect(bookings.getByText('Emma Bennett',{exact:true})).toHaveCount(0);
  await page.goto('/salon/dashboard/photos');
...
for(const [locale,width,height] of [['en',390,844],['fr',768,1024],['es',1440,1000],['zh-CN',844,390]] as const){
 test(`Master demo private page recovers and keeps sample facts in ${locale}`,async({page},info)=>{
  const f=await p0OwnerFixture(page,{populated:true,locale});Object.assign(f.business,{is_demo:true,name:'Culture House'});
  ...
  await page.route('**/api/salon/demo-page',route=>{
   ...
   return route.fulfill(requests===1?{status:503,json:{error:'The page preview could not load. Try again.',request_id:'DEMO-RETRY'}}:{json:{sample:true,policy:{id:'sample-policy',version:2,source_locale:'en',policy:{...POLICY_DEFAULTS,business_policy_text:'Please arrive with detangled hair.'}},assistant:{id:f.business.id,userId:f.session.user.id,isOwner:true,permissions:null,avatar:'woman'},business:{name:f.business.name,description:'Protective styling and healthy hair.',cover:'/images/salon-warm.jpg',city:'New York',photos:[]},services:[{id:f.ids.service,name:'Boho / Knotless Braids',base_price:210,duration_min_hours:4,duration_max_hours:4,description:'Comfortable braids and a natural finish.'}],team:[{id:f.ids.professional,name:'Amara Bennett',bio:'Gentle styling and scalp care.'}]}});
  });
  ...
  await expect(page.getByRole('main').getByRole('alert')).toContainText(t('The page preview could not load. Try again.'));
  await page.getByRole('button',{name:t('Try again'),exact:true}).click();
  await expect(page.getByRole('heading',{name:'Culture House',exact:true})).toBeVisible();
  await expect(page.getByText('Boho / Knotless Braids',{exact:true})).toBeVisible();
  await expect(page.getByRole('heading',{name:t('Page preview'),exact:true})).toBeVisible();
  await expect(page.getByText('Private demonstration — sample data',{exact:true})).toHaveCount(0);
  await expect(page.getByRole('button',{name:/Book now|Pay now/i})).toHaveCount(0);
  await page.locator('#business-policies summary').click();await expect(page.getByText('Please arrive with detangled hair.',{exact:true})).toBeVisible();
  await page.reload();await expect(page.getByRole('heading',{name:'Amara Bennett',exact:true})).toBeVisible();
...
test('Master demo Assistant professional removal requires explicit review from Overview',async({page})=>{
 ...
 await page.route('**/api/salon/assistant',route=>{
  ...
  if(b.action==='confirm'){expect(b.request_id).toBe(id);expect(b.digest).toBe('a'.repeat(64));writes++;return route.fulfill({json:{verified:true,result:{professional_name:'Amara Bennett',archived:true,history_preserved:true}}});}
  id=b.request_id;return route.fulfill({json:{preview_required:true,request:{id,tool:'prepare_professional_archive',risk_class:4,digest:'a'.repeat(64),arguments:{stylist_id:f.ids.professional},execution_payload:{professional_name:'Amara Bennett',archive:true,preserve_history:true},before_summary:{},confirmed_at:null}}});
 });
 ...
 await dialog.locator('textarea').fill('Remove Amara Bennett from my team');
