...
for(const [locale,width,height]of [['en',390,844],['fr',768,900],['es',1440,900],['zh-CN',844,390]] as const){
 test(`Master controls review from Overview retains settings and exact failed-request retry in ${locale}`,async({page},info)=>{
  ...
  const cases=[{section:'deposits',changes:{rate:20,threshold_amount:200,threshold_rate:30}},{section:'growth',changes:{reminder_hours:[48,2],waitlist_service_ids:[f.ids.service]}},{section:'rebooking',changes:{enabled:true,absence_days:60,minimum_visits:2,service_ids:[f.ids.service]}},{section:'profile',changes:{name:'Original Business Name',phone:'+12125550123',languages:['French','Spanish']}},{section:'notifications',changes:{marketing:false}},{section:'booking',changes:{slot_minutes:15,buffer_minutes:30}},{section:'location',changes:{home_address_public:true,travel_fee_cents:1250}}];
  ...
  for(index=0;index<cases.length;index++){
   ...
   const notice=({deposits:copy.deposit_notice,growth:copy.growth_notice,rebooking:copy.contact_notice,notifications:copy.notification_notice,booking:copy.deposit_notice,location:copy.location_notice} as Record<string,string>)[cases[index].section];if(notice)await expect(preview).toContainText(notice);
   if(index===1||index===2)await expect(preview.getByText('Original service GC51',{exact:true})).toBeVisible();
   if(cases[index].section==='profile')await expect(preview.getByText('Original Business Name',{exact:true})).toBeVisible();
   if(cases[index].section==='location'){await expect(preview).toContainText('123 Fictional Street, Brooklyn, NY, 11201');await expect(preview).toContainText(new Intl.NumberFormat(locale,{style:'currency',currency:'USD'}).format(12.5));}
  ...
  expect(attempts).toBe(8);await page.reload();await open();await send('Read my saved deposit rate.');await expect(dialog.getByText(`${copy.rate}: 20%.`,{exact:true})).toBeVisible();expect(writes).toBe(7);
