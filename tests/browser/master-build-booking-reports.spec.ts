...
for(const [locale,index,width,height] of [['en',0,390,844],['fr',1,768,900],['es',2,1440,1000],['zh-CN',3,844,390]] as const){
 test(`Master booking report preserves entitlement and scoped facts in ${locale} at ${width}`,async({page},info)=>{
  ...
  let releaseTranslations!:()=>void;const translations=new Promise<void>(resolve=>{releaseTranslations=resolve;});
  await page.route('**/api/i18n?*',async route=>{await translations;await route.fulfill({json:{locale,sourceMessages:{'Monthly appointment report':t('Monthly appointment report')+' · Engine'}}});});
  const translationResponse=page.waitForResponse(response=>response.url().includes('/api/i18n?'));
  ...
  const report=page.getByRole('region',{name:new RegExp('^'+t('Monthly appointment report'))});
  try{await expect(report.getByRole('alert')).toContainText('11000000-0000-4000-8000-000000000099');}finally{releaseTranslations();}
  await translationResponse;await expect(report.getByRole('heading',{name:t('Monthly appointment report')+' · Engine',exact:true})).toBeVisible();await expect(report.getByRole('alert')).toContainText('11000000-0000-4000-8000-000000000099');expect(reads).toBe(1);
