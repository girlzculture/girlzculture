...
for (const [locale, width, height] of [
  ...
  test(`P0 Assistant spoken controls and text fallback in ${locale}`, async ({ page }) => {
    ...
    await page.addInitScript(() => {
      ...
      Object.defineProperty(window, 'speechSynthesis', { configurable: true, value: {
        getVoices: () => ['en-US', 'fr-FR', 'es-ES', 'zh-CN'].map(lang => ({ lang, localService: true })),
    ...
    if (locale === 'wo') {
      ...
      expect(await calls()).toEqual([`speak:${{ en: 'en-US', fr: 'fr-FR', es: 'es-ES', 'zh-CN': 'zh-CN' }[locale]}`, 'pause', 'resume', 'cancel']);
      await dialog.getByRole('button', { name: t('Read aloud'), exact: true }).click();
