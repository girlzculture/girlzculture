-- Category and city filters are applied before counting and pagination.
-- Preserve the old RPC for rollback and existing integrations.
begin;
...
begin
  ...
  with prepared as (
    ...
    where (
      ...
      and (p_business_types is null or lower(p.business_type) = any(p_business_types))
      and (nullif(p_city_filter, '') is null or lower(p.address_city) = lower(p_city_filter))
...
revoke all on function public.admin_list_businesses(
  uuid,text,text,uuid,text,text,numeric,boolean,double precision,
  double precision,double precision,text,text,boolean,text,text,integer,integer,text[],text
...
create index if not exists salons_directory_category_city_idx
on public.salons (lower(business_type), address_state, lower(address_city), name, id)
