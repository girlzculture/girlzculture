...
begin
 ...
 if permission='styles' then
  ...
  if changes ? 'availability' then
   if changes->'availability'='null'::jsonb then vals:=jsonb_set(vals,'{availability}','{}');end if;
   if not public.p0_actor_has_permission(p_salon,p_actor,'availability') then raise exception 'ASSISTANT_ACCESS_DENIED';end if;
   if changes->'availability'<>'null'::jsonb then
    for key,choice in select * from jsonb_each(changes->'availability') loop
     if choice->'closed'='false' and (choice->>'open' is null or choice->>'close' is null or choice->>'open'>=choice->>'close') then raise exception 'ASSISTANT_INVALID_INPUT';end if;
    end loop;
   end if;
  end if;
