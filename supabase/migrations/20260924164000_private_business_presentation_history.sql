...
begin
 ...
 for i in 1..4 loop
  ...
  values(gc_private.demo_record_id(p_salon,'product',i),p_salon,v_products[i],case i when 1 then 'Gentle cleansing for hydrated hair.' when 2 then 'Daily moisture and easy detangling.' when 3 then 'Lightweight nourishment for the scalp.' else 'Protect your style overnight.' end,12+i*3,'CH-'||lpad(i::text,3,'0'),360,true,10,'Active',true,true,false,v_start);
 end loop;
 for i in 1..480 loop
