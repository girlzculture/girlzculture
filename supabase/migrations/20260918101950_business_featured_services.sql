...
-- An owner's editorial selection is independent of booking popularity.
alter table public.styles add column is_featured boolean not null default false;
...
begin
  ...
  if p_style_id is not null then
    ...
    update public.styles set
      ...
      is_featured = v_style.is_featured,
    ...
    insert into public.styles(
      ...
      coalesce(v_style.is_draft, false), v_style.archived_at, coalesce(v_style.is_featured, false)
    ) returning * into v_style;
  end if;
...
update public.engine_settings set published_value='"20260918101950"'::jsonb,draft_value='"20260918101950"'::jsonb,updated_at=now() where setting_key='integrations.expected_migration';
commit;
